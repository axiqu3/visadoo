...
- **STANDING FRONTEND RULE — global "Chat with us" WhatsApp button:** `branding.js` auto-injects the floating WhatsApp button (`.wa-float`, fixed bottom-right, styled by `styles.css`) on any customer-facing page that loads `branding.js` and doesn't already hard-code one. **All future customer-facing frontend pages must include the same global button in the same fixed position/layout via this shared component** — in practice, just make sure the page loads `branding.js` (every public page and edge function already does) and it appears automatically, with the correct WhatsApp link/label from `site_settings`. It is hidden on the admin console via `.has-admin-side .wa-float{display:none}` in `app.css`. Do not hand-code a second one.
...
- **03 Jul 2026** — **SEO improvements (from external audit)**: (1) **Image descriptions (alt text)** — every image upload now has a description box (article cover, images inserted into articles, destination image, event image, homepage banner); these are added to the live pages for Google Images and screen readers, defaulting to the item's name if left blank. (2) **Editable web addresses** — Articles and Content pages now let you hand-edit the address (slug); when changed, the old address **automatically forwards (301)** to the new one, so shared links and Google rankings don't break. Verified live. Note: three audit items (canonical tags, Google Analytics + Search Console fields, single H1 per page) were found to **already exist** and were left unchanged.

- **02 Jul 2026** — **Admin Dashboard (Phase 1)**: new "Dashboard" menu item (top of the admin area; the default screen staff land on). Read-only. Tabs: Overview, Leads, Operations, Staff performance, and Revenue (Admin/Finance only). Filters: period (default This month), staff (Admin/Finance only), country, lead source. Role safety: money/revenue shown only to Admin & Finance; sales/operations/viewer staff see their own performance only and no money; customers can't open it. Shows real data (leads, enquiries, applications, payments, refunds, staff activity). No supplier cost/profit/margin, no passport/document details, nothing editable.

- **02 Jul 2026** — **Mobile-number verification (WhatsApp OTP) — LIVE & mandatory**: public applicants must verify their mobile number with a one-time WhatsApp code (approved `otp_verification` template) before they can submit. Field is "Mobile number" with a country picker (default +91); code valid 10 min, resend after 30s, 5 tries. If the code can't be received, submission is blocked but a "Contact us on WhatsApp" help line is shown (and staff can apply on their behalf, which skips OTP). Staff see a "Mobile ✓" badge on verified applications. Tested end-to-end to the owner's WhatsApp before switch-on.

- **02 Jul 2026** — **Global "Chat with us" WhatsApp button**: `branding.js` now auto-injects the exact homepage floating button on every customer-facing page (country, visa, article, content page, events, event detail, apply/track) that doesn't already have one — no duplicates, same fixed position/wording/icon/link. Future frontend pages inherit it automatically. Hidden on the admin console (one `app.css` rule). No DB/RLS/messaging changes. _(Fix same day: added a WhatsApp-number fallback in `branding.js` so the button also shows on the server-rendered pages, which don't load `config.js`; Brand & Settings' WhatsApp number still overrides it.)_
