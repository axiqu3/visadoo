...
_Bugfix (2026-06-25): **"View history" bounce fixed.** Clicking a customer opened `custview` but the `hashchange` listener re-ran `resolveStartView()`, which didn't list `custview` → bounced to Applications. FIX (2 spots, no other changes): `openCustomer` now sets hash `custview/<id>` (id persisted, so refresh/bookmark reopens the customer); `resolveStartView` parses `custview/<id>` → sets `custViewId`, returns 'custview' (matches state.view → no bounce). Verified via routing logic tests. Tags `before-fix-view-history`, `after-fix-view-history`._

_Latest (2026-06-25): **"Action Needed" two-way exchange DONE.** Staff set status to "Action Needed" + write a **Message to customer** and **Documents to request** (one per line) → saved as a staff row in new table `app_messages` (RLS). Customer tracker (`appCard`) shows the request + a **labelled upload slot per requested doc** + a message box (`submitReply`): files go to `visa-documents/<uid>/<appId>/reply_*`, a `customer` `app_messages` row is inserted (attachments jsonb [{name,path,label}]). DB trigger `on_customer_reply` (SECURITY DEFINER) auto-moves the app `Action Needed→Under Review` + sets new `applications.unread_reply` flag. Staff see a **conversation thread** (`threadHtml`) on the admin card + a **"New reply" badge** (cleared on Save); customers + staff open attachments via signed URLs (`wireThreadFiles`). Customer alert email already existed (send-status-email "Action Needed"); new **`notify-staff-reply`** edge fn emails staff (site contact_email) when a customer replies. **RLS verified:** a customer can only read/write their OWN app's thread, cannot forge a `staff` message, cannot post to others' apps; trigger + badge confirmed. Migration `action_needed_thread`; edge fn `notify-staff-reply`. Files application.js, app.css. Checkpoints `before-action-needed`, `after-action-needed`. NOTE committed, deploy separately. (Note: prod now has real new applications incl. one team-set to "Action Needed" — do not delete live data.)_

_Earlier (2026-06-25): **Per-customer communication history DONE.** Clicking a customer (Customers list → "View history →", cards now clickable) opens a dedicated **customer page** (view `custview`, `renderCustomerDetail`/`paintCustomerDetail`/`msgRowHtml`, `custViewId`, `openCustomer`): profile header (name/email/phone/country, lead source, Marketing On/Off + toggle, "Since" date), their **Applications** list, and their full **Message history** (subject, template, channel, delivery-status pill, date, expandable "View content"). Reads existing `messages`/`applications`/`consent` (status emails + automations already logged); **no DB change**. Will also show WhatsApp once integrated. Admin-only; verified message log stays staff-only (customer sees 0). Checkpoints `before-comm-history`, `after-comm-history`. Built while waiting on Telinfy. NOTE committed, deploy separately._

_Earlier (2026-06-24): **Backend nav redesigned — grouped LEFT SIDEBAR** (replaces the crowded horizontal tab row). Same `data-section` keys + routing (nothing broke). Built in application.js `adminNavModel()` (groups → items, each role-gated) + `adminSections()`/`wireAdminSections()` + `ADMIN_VIEWS`; styled in app.css (`.admin-side`, `.side-group`, `.side-item`, etc.). Groups: **Customers** (Applications/Enquiries/Customers), **Messaging** (Communications), **Catalogue** (Destinations/Visa Types), **Content** (Articles/Content/Site SEO), **Settings** (Brand & Settings/Email/Team). Collapsible groups (current auto-opens), inline-SVG icons in brand colour, role-aware (empty groups hide). Desktop: fixed sidebar (top:73px, 236px) + content shifted via `body.has-admin-side`. Phone (≤900px): ☰ toggle opens a slide-out drawer + backdrop. Top bar (logo/role/sign out) unchanged; customer & public pages untouched. **To add a future admin feature: add it to a group in `adminNavModel()` + the router — no horizontal-space worries.** Verified desktop + mobile via preview. Checkpoints `before-admin-sidebar`, `after-admin-sidebar`. NOTE: committed, deploy separately._
