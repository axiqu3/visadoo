...
_Latest (2026-06-23): **Visa ETA feature COMPLETE** — admin sets a per-visa processing time (days/hours, columns visa_types.processing_time_value/unit); shown on the apply form, the visa detail page, AND as a brand-coloured "⚡ Get your visa in about X days/hours" badge on each visa card on the **country pages** (country.js etaBadge; hidden when blank). All branding flash issues fixed. **A fresh chat for the next modules should start only AFTER this work is pushed to GitHub (GitHub Desktop → Push origin).**_
