(function () {
  ...
  var UI_COPY={
    ...
    hi:{explore:'एक्सप्लोर',events:'इवेंट्स',articles:'लेख',track:'वीज़ा ट्रैक करें',assistant:'AI यात्रा सहायक',online:'अभी ऑनलाइन',hello:'नमस्ते! मैं वीज़ा विकल्प, दस्तावेज़, शुल्क और ट्रैकिंग में मदद कर सकता हूँ।',placeholder:'वीज़ा के बारे में पूछें…',send:'भेजें'},
  ...
  function translateVisaValue(value,language){
    ...
    var exact={
      ml:{'Single Entry':'ഒറ്റ പ്രവേശനം','Multiple Entry':'ഒന്നിലധികം പ്രവേശനം','Flexible stay':'സൗകര്യപ്രദമായ താമസം','Confirmed after review':'പരിശോധനയ്ക്ക് ശേഷം സ്ഥിരീകരിക്കും'},
      hi:{'Single Entry':'सिंगल एंट्री','Multiple Entry':'मल्टीपल एंट्री','Flexible stay':'लचीला ठहराव','Confirmed after review':'समीक्षा के बाद पुष्टि'},
      ar:{'Single Entry':'دخول مفرد','Multiple Entry':'دخول متعدد','Flexible stay':'إقامة مرنة','Confirmed after review':'يُؤكد بعد المراجعة'},
      fr:{'Single Entry':'Entrée simple','Multiple Entry':'Entrées multiples','Flexible stay':'Séjour flexible','Confirmed after review':'Confirmé après examen'},
      es:{'Single Entry':'Entrada única','Multiple Entry':'Entradas múltiples','Flexible stay':'Estancia flexible','Confirmed after review':'Confirmado tras revisión'}
    };
  ...
  function applyPageLanguage(language){
    ...
    navLinks.forEach(function(node,index){
      if(copy.nav[index]) node.textContent=copy.nav[index];
    });
    ...
    sectionHeads.forEach(function(section,index){
      ...
      if(span && values[0]) span.textContent=values[0];
      if(title) title.textContent=values[1];
      if(description && sectionKey !== 0) description.textContent=values[2];
    });
    ...
    reqAccordions.forEach(function(item,index){
      ...
      if(summary) summary.textContent=values[0];
    ...
    stepItems.forEach(function(item,index){
      ...
      if(description) {
        if (index === 2) {
          ...
          description.innerHTML = values[1].replace('{date}', '<strong data-uae-delivery-date>' + dateStr + '</strong>');
        } else {
    ...
    faqItems.forEach(function(item,index){
      ...
      var body=item.querySelector('.uae-travel-accordion-body p, p');
      if(summary) setDirectText(summary,values[0]);
      if(body) body.textContent=values[1];
    });
    ...
    if (benefitsSection && copy.benefitsList) {
      ...
      if (heading) heading.textContent = copy.benefitsList[0][0];
  ...
  function initAssistant(countryName){
    ...
    function answer(question){
      ...
      if(/track|status|ട്രാക്ക്|स्थिति/.test(q)) return 'Open “Track visa” from the top menu. You can see the latest status, required actions and updates there.';
    ...
    function submitQuestion(question){
      ...
      window.setTimeout(function(){bot.classList.remove('ai-message-loading');bot.textContent=answer(value);conversation.scrollTop=conversation.scrollHeight;},420);
    }
  ...
  function ensureCountryWhatsApp(countryName){
    var link=document.querySelector('.wa-float');
    if(!link){
    ...
    link.href='https://wa.me/'+number+'?text='+encodeURIComponent('Hi Visa Doo, I have a question about a '+countryName+' visa.');
  ...
  function initTravellerSelector(){
    ...
    continueButton.addEventListener('click',function(){
      ...
      if(selectedMode === 'multiple') {
        ...
        var nameInput=detailsWrap.querySelector('[data-traveller-name="0"]');
        if(nameInput && nameInput.value) {
          people.push({name:nameInput.value,relation:'Self'});
        }
      }
