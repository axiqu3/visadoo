...
export default async () => {
  try {
    ...
    const events = await fetchJson(SUPABASE_URL+"/rest/v1/events?active=eq.true&select=slug,updated_at");

    const urls = [];
    urls.push({ loc: SITE+"/", pri: "1.0" });
    urls.push({ loc: SITE+"/events", pri: "0.8" });
    urls.push({ loc: SITE+"/articles", pri: "0.7" });
    (visas||[]).forEach(function(v){ urls.push({ loc: SITE+"/visa/"+v.slug, lastmod: v.updated_at, pri: "0.9" }); });
    (events||[]).forEach(function(e){ urls.push({ loc: SITE+"/event/"+e.slug, lastmod: e.updated_at, pri: "0.8" }); });
