...
(function () {
  ...
  function renderHeader(){
    ...
    var bell=document.getElementById('adminNotifyBell');
    if(bell){
      sb.from('applications').select('id,full_name,reference_code,created_at,status').eq('status','Submitted').order('created_at',{ascending:false}).limit(20).then(function(r){
        if(r.error) return; var rows=r.data||[]; var seen=[]; try{seen=JSON.parse(localStorage.getItem('visadoo-admin-seen-apps')||'[]');}catch(_e){}
        var fresh=rows.filter(function(a){return seen.indexOf(a.id)<0;}); var badge=document.getElementById('adminNotifyCount');
        if(badge&&fresh.length){badge.hidden=false;badge.textContent=String(fresh.length);badge.title=fresh.map(function(a){return a.full_name||a.reference_code;}).join(', ');}
        bell.onclick=function(){ localStorage.setItem('visadoo-admin-seen-apps',JSON.stringify(rows.map(function(a){return a.id;}))); go('admin'); };
      });
    }
  }
  ...
  // ============================================================
  //  JAPAN REVIEW & ACCORDION FLOW HELPERS
  // ============================================================
  function getStandardStep3Html(meta, photoHeading, step3Heading, chosen, isMultiple, travellerIndex, defaultName) {
    return '<div class="passport-review-layout">'+
        ...
        '<section class="passport-review-form apply-step active">' +
          ...
          '<div class="grid2">' +
            ...
            '</div>' +
            ((Number(qParam('travellers'))||1)>1 && (Number(qParam('travellerIndex'))||0)>0 ?
              '<div class="review-contact-details group-contact-note"><h3>Contact Details</h3><p>This group uses the primary applicant’s verified email and phone number for all visa updates.</p></div>' :
              '<div class="review-contact-details">'+
                ...
                '<div class="review-contact-fields">'+
                  ...
                  '<div class="field review-phone-field" id="mobileField"><label for="phone">Phone number <span class="req-star">*</span></label><input id="phone" name="phone" type="tel" autocomplete="tel" required></div>'+
        ...
        '<section class="passport-review-form apply-step active" style="border:none;box-shadow:none;background:transparent;padding:0;margin:0">'+
          ...
          '<div class="wizard-progress-card">'+
            ...
            '<div class="wizard-steps-grid">'+
              ...
              '<button type="button" class="wizard-step-node" data-step="6">'+
                ...
                '<span class="step-node-label">Japan-specific details</span>'+
          ...
          '<div class="japan-accordion-container">'+
            ...
            '<div class="review-section-card" data-section="3" style="display:none">'+
              ...
              '<div class="review-section-body">'+
                '<div class="grid2">'+
                  '<div class="field"><label for="purpose">Purpose of Visit <span class="req-star">*</span></label><select id="purpose" required><option value="">Select purpose</option><option value="Tourism">Tourism</option><option value="Business">Business</option><option value="Visiting Family/Friends">Visiting Family/Friends</option><option value="Conference">Conference</option><option value="Other">Other</option></select></div>'+
                  field('arrival_date','Date of Arrival in Japan','date','',true) +
                  field('stay_duration','Intended Length of Stay','text','',true) +
                  field('port_of_entry','Port of Entry','text','',true) +
                  field('airline','Airline / Flight Name or Number','text','',true) +
                  field('hotel_name','Hotel / Host Name','text','',true) +
                  field('hotel_phone','Hotel / Host Phone','text','',true) +
                  '<div class="field"><label for="previous_japan_stay">Previous Stay in Japan <span class="req-star">*</span></label><select id="previous_japan_stay" required><option value="No">No</option><option value="Yes">Yes</option></select></div>'+
                '</div>'+
            ...
            '<div class="review-section-card" data-section="6" style="display:none">'+
              '<div class="review-section-card-header">'+
                '<h3>Japan-specific details</h3>'+
                '<span class="status-badge" id="badge_sec6"></span>'+
              '</div>'+
              '<div class="review-section-body">'+
                
                '<div class="japan-subsection" id="guarantor_section" style="margin-top:0;padding-top:0;border-top:none">'+
                  '<div class="japan-subsection-header">'+
                    '<h4>A. GUARANTOR / REFERENCE IN JAPAN</h4>'+
                    '<label class="na-checkbox-label"><input type="checkbox" id="guarantor_na"> [ Not Applicable ]</label>'+
                  '</div>'+
                  '<div class="grid2">'+
                    ...
                    field('guarantor_immigration_status','Immigration Status','text','',true) +
                  '</div>'+
                  '<div class="field guarantor-field" style="width:100%"><label for="guarantor_address">Address <span class="req-star">*</span></label><textarea id="guarantor_address" style="min-height:64px" required placeholder="Enter guarantor address"></textarea></div>'+
                '</div>'+
 
                '<div class="japan-subsection" id="inviter_section">'+
                  '<div class="japan-subsection-header">'+
                    '<h4>B. INVITER IN JAPAN</h4>'+
                    '<div style="display:flex;gap:8px;align-items:center">'+
                      '<button type="button" class="action-btn-sm" id="inviter_same_btn">[ Same as Guarantor ]</button>'+
                      '<label class="na-checkbox-label"><input type="checkbox" id="inviter_na"> [ Not Applicable ]</label>'+
                    '</div>'+
                  '</div>'+
                  '<div class="grid2">'+
                    ...
                    field('inviter_immigration_status','Immigration Status','text','',true) +
                  '</div>'+
                  '<div class="field inviter-field" style="width:100%"><label for="inviter_address">Address <span class="req-star">*</span></label><textarea id="inviter_address" style="min-height:64px" required placeholder="Enter inviter address"></textarea></div>'+
                '</div>'+
 
                '<div class="japan-subsection" id="declarations_section">'+
                  '<div class="japan-subsection-header"><h4>C. JAPAN DECLARATIONS</h4></div>'+
                  '<div class="dec-list">'+
                    '<div class="dec-row"><div class="dec-question">1. Have you ever been convicted of a crime or offence in any country? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_crime" value="Yes" required> Yes</label><label><input type="radio" name="dec_crime" value="No" required> No</label></div></div>'+
                    '<div class="dec-row"><div class="dec-question">2. Have you ever been sentenced to imprisonment for one year or more? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_prison" value="Yes" required> Yes</label><label><input type="radio" name="dec_prison" value="No" required> No</label></div></div>'+
                    '<div class="dec-row"><div class="dec-question">3. Have you ever been deported or removed from Japan or another country for overstaying a visa or violating immigration law? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_deport" value="Yes" required> Yes</label><label><input type="radio" name="dec_deport" value="No" required> No</label></div></div>'+
                    '<div class="dec-row"><div class="dec-question">4. Have you ever been convicted of a drug-related offence? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_drug" value="Yes" required> Yes</label><label><input type="radio" name="dec_drug" value="No" required> No</label></div></div>'+
                    '<div class="dec-row"><div class="dec-question">5. Have you ever engaged in prostitution or related activities? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_prostitution" value="Yes" required> Yes</label><label><input type="radio" name="dec_prostitution" value="No" required> No</label></div></div>'+
                    '<div class="dec-row"><div class="dec-question">6. Have you ever committed, assisted or incited trafficking in persons? <span class="req-star">*</span></div><div class="dec-options"><label><input type="radio" name="dec_trafficking" value="Yes" required> Yes</label><label><input type="radio" name="dec_trafficking" value="No" required> No</label></div></div>'+
                  '</div>'+
                  '<div class="field" id="dec_details_wrap" style="display:none;width:100%;margin-top:16px"><label for="dec_details">Please provide details <span class="req-star">*</span></label><textarea id="dec_details" style="min-height:80px" placeholder="Provide details here..."></textarea></div>'+
                '</div>'+
                '<div class="wizard-navigation-footer">'+
                  '<button type="button" class="btn btn-ghost wizard-btn-back"><span aria-hidden="true">←</span> Back</button>'+
                  '<button type="button" class="btn btn-primary wizard-btn-next">Next <span aria-hidden="true">→</span></button>'+
  ...
  function isStepNavigable(s) {
    if (s === activeWizardSection) return true;
    if (s < activeWizardSection) return true;
    
    for (var i = 1; i < s; i++) {
      var badge = document.getElementById('badge_sec' + i);
      var isPrevComplete = badge && badge.classList.contains('complete');
  ...
  function goToWizardSection(secNum) {
    ...
    cards.forEach(function(card) {
      ...
      if (sectionIndex === secNum) {
        ...
        card.classList.remove('active-wizard-step');
        card.setAttribute('hidden', 'hidden');
        card.style.display = 'none';
      }
    });

    var formContainer = document.querySelector('.passport-review-content');
    if (formContainer) {
  ...
  function clearCardErrors(card) {
    card.querySelectorAll('.field.is-invalid').forEach(function(f) {
      f.classList.remove('is-invalid');
      var err = f.querySelector('.error-msg');
      if (err) err.remove();
    });
    card.querySelectorAll('.dec-row.is-invalid').forEach(function(r) {
      r.classList.remove('is-invalid');
      var err = r.querySelector('.error-msg');
      if (err) err.remove();
    });
  }
  ...
  function showRadioError(name, msg) {
    ...
    if (!radio) return;
    var row = radio.closest('.dec-row');
    if (!row) return;
    row.classList.add('is-invalid');
    var err = row.querySelector('.error-msg');
    if (!err) {
      err = document.createElement('div');
      err.className = 'error-msg';
      err.style.color = 'var(--red)';
  ...
  function validateJapanSection(secNum) {
    var card = document.querySelector('.review-section-card[data-section="'+secNum+'"]');
    if (!card) return true;
    ...
    if (secNum === 7) {
      if (picked.passport || picked.passport_back) {
        if (!picked.passport) {
          isValid = false;
          var btn = card.querySelector('#viewPassportBtn') || card.querySelector('#replacePassportBtn');
          if (btn) {
            showFieldError(btn, 'Passport front page copy is required.');
            if (!firstInvalidElement) firstInvalidElement = btn;
          }
        }
        if (!picked.passport_back) {
          isValid = false;
          var btn = card.querySelector('#replacePassportBtn');
          if (btn) {
            showFieldError(btn, 'Passport back page copy is required.');
            if (!firstInvalidElement) firstInvalidElement = btn;
          }
        }
      }
      if (!picked.photo) {
        isValid = false;
        var btn = card.querySelector('#viewPhotoBtn') || card.querySelector('#replacePhotoBtn');
        if (btn) {
          showFieldError(btn, 'Personal photograph is required.');
          if (!firstInvalidElement) firstInvalidElement = btn;
        }
      }
      var appDate = document.getElementById('application_date');
      if (appDate && !appDate.value.trim()) {
        isValid = false;
        showFieldError(appDate, 'Application date is required.');
        if (!firstInvalidElement) firstInvalidElement = appDate;
      }
    } else if (secNum === 4) {
      ...
      fields.forEach(function(el) {
        ...
        if (el.type === 'radio') {
          var name = el.name;
          if (!name || checkedRadios[name]) return;
          checkedRadios[name] = true;
          
          var checkedOption = card.querySelector('input[name="'+name+'"]:checked');
          if (!checkedOption && el.hasAttribute('required')) {
            isValid = false;
            showRadioError(name, 'Selecting an option is required.');
          ...
          if (el.hasAttribute('required') && !el.value.trim()) {
            isValid = false;
            var labelText = '';
            var label = card.closest('form').querySelector('label[for="'+el.id+'"]');
            if (label) {
              labelText = label.textContent.replace('*', '').trim();
            } else {
              labelText = el.placeholder || el.id;
            }
      ...
      if (mobileOtpRequired && !mobileVerified) {
        isValid = false;
        var phoneInput = card.querySelector('#phone');
        if (phoneInput) {
          showFieldError(phoneInput, 'Mobile number verification is required.');
  ...
  function wireJapanAccordion() {
    var cards = document.querySelectorAll('.review-section-card');
    cards.forEach(function(card) {
      var header = card.querySelector('.review-section-header');
      if (header) {
  ...
  function wireJapanWorkflow() {
    // 1. Previous stays toggle
    ...
    // 3. Guarantor N/A Toggle
    var gNa = document.getElementById('guarantor_na');
    if (gNa) {
      gNa.onchange = function() {
        var disabled = gNa.checked;
        var section = document.getElementById('guarantor_section');
        var fields = section.querySelectorAll('input, select, textarea');
        fields.forEach(function(f) {
          if (f.id === 'guarantor_na') return;
          var fWrap = f.closest('.field');
          if (disabled) {
    ...
    // 4. Inviter Same/NA Toggle
    var iNa = document.getElementById('inviter_na');
    if (iNa) {
      iNa.onchange = function() {
        var disabled = iNa.checked;
        var section = document.getElementById('inviter_section');
        var fields = section.querySelectorAll('input, select, textarea');
        fields.forEach(function(f) {
          if (f.id === 'inviter_na') return;
          var fWrap = f.closest('.field');
          if (disabled) {
            f.setAttribute('disabled', 'disabled');
            f.removeAttribute('required');
    ...
    var iSame = document.getElementById('inviter_same_btn');
    if (iSame) {
      iSame.onclick = function(e) {
        e.preventDefault();
        if (iNa && iNa.checked) return;
        var pairs = [
          ['guarantor_name', 'inviter_name'],
          ['guarantor_phone', 'inviter_phone'],
          ['guarantor_dob', 'inviter_dob'],
          ['guarantor_gender', 'inviter_gender'],
          ['guarantor_relationship', 'inviter_relationship'],
          ['guarantor_occupation', 'inviter_occupation'],
          ['guarantor_nationality', 'inviter_nationality'],
          ['guarantor_immigration_status', 'inviter_immigration_status'],
          ['guarantor_address', 'inviter_address']
        ];
        pairs.forEach(function(p) {
          var src = document.getElementById(p[0]);
          var dest = document.getElementById(p[1]);
          if (src && dest) {
    ...
    // 5. Declarations details toggle
    var decRadios = document.querySelectorAll('input[type="radio"][name^="dec_"]');
    var decWrap = document.getElementById('dec_details_wrap');
    var decText = document.getElementById('dec_details');
    if (decRadios.length && decWrap && decText) {
      decRadios.forEach(function(r) {
        r.onchange = function() {
          var anyYes = false;
          var names = ['dec_crime', 'dec_prison', 'dec_deport', 'dec_drug', 'dec_prostitution', 'dec_trafficking'];
          names.forEach(function(n) {
            var ch = document.querySelector('input[name="'+n+'"]:checked');
            if (ch && ch.value === 'Yes') anyYes = true;
          });
          if (anyYes) {
            decWrap.style.display = 'block';
            decText.setAttribute('required', 'required');
          } else {
            decWrap.style.display = 'none';
            decText.removeAttribute('required');
            decText.value = '';
          }
          updateAllJapanSectionStatuses();
        };
    ...
    // 6. Doc previews and replace links
    ...
    var sigDrawBtn = document.getElementById('sig_draw_btn');
    var sigUploadBtn = document.getElementById('sig_upload_btn');
    var sigDrawPanel = document.getElementById('sig_draw_panel');
    var sigUploadPanel = document.getElementById('sig_upload_panel');
    var sigClearBtn = document.getElementById('sig_clear_btn');
    var sigSaveBtn = document.getElementById('sig_save_btn');
    var sigCanvasStatus = document.getElementById('sig_canvas_status');
    var sigPreviewBox = document.getElementById('sig_preview_box');
    var sigPreviewImg = document.getElementById('sig_preview_img');
    var sigFileInput = document.getElementById('file_signature');
    var sigDropZone = document.getElementById('drop_signature');
    var sigFName = document.getElementById('fname_signature');

    if (sigDrawBtn && sigUploadBtn && sigDrawPanel && sigUploadPanel) {
      sigDrawBtn.onclick = function(e) {
        e.preventDefault();
        sigDrawPanel.style.display = 'block';
        sigUploadPanel.style.display = 'none';
        sigDrawBtn.style.background = 'var(--blue-600)';
        sigDrawBtn.style.color = '#fff';
        sigUploadBtn.style.background = 'var(--blue-100)';
        sigUploadBtn.style.color = 'var(--blue-600)';
      };
      sigUploadBtn.onclick = function(e) {
        e.preventDefault();
        sigDrawPanel.style.display = 'none';
        sigUploadPanel.style.display = 'block';
        sigUploadBtn.style.background = 'var(--blue-600)';
        sigUploadBtn.style.color = '#fff';
        sigDrawBtn.style.background = 'var(--blue-100)';
        sigDrawBtn.style.color = 'var(--blue-600)';
      };
    }
    ...
    if (canvas) {
      ...
      function drawMove(e) {
        ...
        ctx.lineJoin = 'round';
        ctx.stroke();
        if (sigCanvasStatus) { sigCanvasStatus.textContent = 'Unsaved changes'; sigCanvasStatus.style.color = 'var(--amber)'; }
      }
      ...
      if (sigClearBtn) {
        sigClearBtn.onclick = function(e) {
          e.preventDefault();
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          if (sigCanvasStatus) { sigCanvasStatus.textContent = 'Cleared'; sigCanvasStatus.style.color = 'var(--muted)'; }
          if (sigPreviewBox) sigPreviewBox.style.display = 'none';
      ...
      if (sigSaveBtn) {
        sigSaveBtn.onclick = function(e) {
          e.preventDefault();
          var buffer = new Uint32Array(ctx.getImageData(0, 0, canvas.width, canvas.height).data.buffer);
          var isBlank = !buffer.some(function(color) { return color !== 0; });
          if (isBlank) { toast('Please draw your signature before saving.'); return; }

          canvas.toBlob(function(blob) {
            var file = new File([blob], 'signature.png', { type: 'image/png' });
            picked.signature = file;
            if (sigPreviewImg) sigPreviewImg.src = URL.createObjectURL(blob);
            if (sigPreviewBox) sigPreviewBox.style.display = 'block';
            if (sigCanvasStatus) { sigCanvasStatus.textContent = 'Saved'; sigCanvasStatus.style.color = 'var(--green)'; }
            toast('Signature saved successfully!');
            updateAllJapanSectionStatuses();
          }, 'image/png');
        };
      }
    ...
    if (sigDropZone && sigFileInput) {
      sigDropZone.onclick = function(e) { e.preventDefault(); sigFileInput.click(); };
      sigFileInput.onchange = function() {
        var file = sigFileInput.files[0];
        if (!file) return;
        if (file.size > 10485760) { toast('Signature file must be under 10 MB.'); return; }
        if (!/\.(jpe?g|png)$/i.test(file.name)) { toast('Please select a JPG or PNG signature image.'); return; }
        picked.signature = file;
        if (sigFName) sigFName.textContent = '✓ ' + file.name;
        if (sigPreviewImg) sigPreviewImg.src = URL.createObjectURL(file);
        if (sigPreviewBox) sigPreviewBox.style.display = 'block';
  ...
  function updateAllJapanSectionStatuses() {
    ...
    // Section 3
    var sec3Reqs = ['purpose', 'arrival_date', 'stay_duration', 'port_of_entry', 'airline', 'hotel_name', 'hotel_phone', 'previous_japan_stay', 'hotel_address'];
    var prevStay = document.getElementById('previous_japan_stay');
    ...
    var gNa = document.getElementById('guarantor_na');
    if (!gNa || !gNa.checked) {
      sec6Empty += countEmpty(['guarantor_name', 'guarantor_phone', 'guarantor_dob', 'guarantor_gender', 'guarantor_relationship', 'guarantor_occupation', 'guarantor_nationality', 'guarantor_immigration_status', 'guarantor_address']);
    }
    var iNa = document.getElementById('inviter_na');
    if (!iNa || !iNa.checked) {
      sec6Empty += countEmpty(['inviter_name', 'inviter_phone', 'inviter_dob', 'inviter_gender', 'inviter_relationship', 'inviter_occupation', 'inviter_nationality', 'inviter_immigration_status', 'inviter_address']);
    }
    var names = ['dec_crime', 'dec_prison', 'dec_deport', 'dec_drug', 'dec_prostitution', 'dec_trafficking'];
    var decSelectedCount = 0;
    var anyYes = false;
    names.forEach(function(n) {
      var ch = document.querySelector('input[name="'+n+'"]:checked');
      if (ch) {
        decSelectedCount++;
        if (ch.value === 'Yes') anyYes = true;
      }
    });
    var decEmpty = 6 - decSelectedCount;
    if (anyYes && (!document.getElementById('dec_details') || !document.getElementById('dec_details').value.trim())) {
      decEmpty++;
    }
    sec6Empty += decEmpty;
    ...
    if (typeof activeWizardSection !== 'undefined') {
      ...
      if (sec1Empty === 0) completedCount++;
      if (sec2Empty === 0) completedCount++;
      if (sec3Empty === 0) completedCount++;
      if (sec4Empty === 0) completedCount++;
      if (sec5Empty === 0) completedCount++;
      if (sec6Empty === 0) completedCount++;
      if (sec7Missing === 0) completedCount++;
      
      var percent = Math.round((completedCount / 7) * 100);
      if (progressPercentageSpan) progressPercentageSpan.textContent = percent + '% complete';
      if (progressBarFill) progressBarFill.style.width = percent + '%';
      
      // Update step nodes
      for (var s = 1; s <= 7; s++) {
        var node = document.querySelector('.wizard-step-node[data-step="'+s+'"]');
        if (!node) continue;
        
        var iconSpan = node.querySelector('.step-node-icon');
        var badgeSpan = document.getElementById('step_node_badge_' + s);
        
        var isComplete = false;
        var missingCount = 0;
        var badgeType = 'field';
        
        if (s === 1) { isComplete = (sec1Empty === 0); missingCount = sec1Empty; }
        else if (s === 2) { isComplete = (sec2Empty === 0); missingCount = sec2Empty; }
        else if (s === 3) { isComplete = (sec3Empty === 0); missingCount = sec3Empty; }
        else if (s === 4) { isComplete = (sec4Empty === 0); missingCount = sec4Empty; }
        else if (s === 5) { isComplete = (sec5Empty === 0); missingCount = sec5Empty; }
        else if (s === 6) { isComplete = (sec6Empty === 0); missingCount = sec6Empty; }
        else if (s === 7) { isComplete = (sec7Missing === 0); missingCount = sec7Missing; badgeType = 'item'; }
        
        node.classList.remove('active', 'completed', 'incomplete');
        
        if (s === activeWizardSection) {
        ...
        if (badgeSpan) {
          if (isComplete) {
            badgeSpan.className = 'step-node-badge completed';
            badgeSpan.textContent = 'Completed';
          } else {
            badgeSpan.className = 'step-node-badge missing';
            badgeSpan.textContent = 'Missing ' + missingCount + ' ' + badgeType + (missingCount === 1 ? '' : 's');
          }
        }
        
        node.disabled = !isStepNavigable(s);
      }
    }
  }

  function validateJapanFieldsBeforeSubmit() {
    var isMultiple = (Number(qParam('travellers'))||1)>1;
    var travellerIndex = Number(qParam('travellerIndex'))||0;
    var isSecondary = isMultiple && travellerIndex > 0;
    ...
    var sec3 = [
      ...
      ['stay_duration', 'Intended length of stay'],
      ['port_of_entry', 'Port of entry'],
      ['airline', 'Airline / flight number'],
      ['hotel_name', 'Hotel name'],
      ['hotel_phone', 'Hotel phone'],
      ['previous_japan_stay', 'Previous stay in Japan selection']
    ];
    ...
    if (!gNa || !gNa.checked) {
      var sec6a = [
        ...
        ['guarantor_phone', 'Guarantor telephone'],
        ['guarantor_dob', 'Guarantor date of birth'],
        ['guarantor_gender', 'Guarantor gender'],
        ['guarantor_relationship', 'Relationship to guarantor'],
        ['guarantor_occupation', 'Guarantor profession / occupation'],
        ['guarantor_nationality', 'Guarantor nationality'],
        ['guarantor_immigration_status', 'Guarantor immigration status'],
        ['guarantor_address', 'Guarantor address']
      ];
      for (i = 0; i < sec6a.length; i++) {
        err = check(sec6a[i][0], sec6a[i][1] + ' is required.', 6);
        if (err) return err;
      }
    }
    var iNa = document.getElementById('inviter_na');
    if (!iNa || !iNa.checked) {
      var sec6b = [
        ...
        ['inviter_phone', 'Inviter telephone'],
        ['inviter_dob', 'Inviter date of birth'],
        ['inviter_gender', 'Inviter gender'],
        ['inviter_relationship', 'Relationship to inviter'],
        ['inviter_occupation', 'Inviter profession / occupation'],
        ['inviter_nationality', 'Inviter nationality'],
        ['inviter_immigration_status', 'Inviter immigration status'],
    ...
    for (i = 0; i < names.length; i++) {
      var ch = document.querySelector('input[name="'+names[i]+'"]:checked');
      if (!ch) {
        return { name: 'Please complete all Japan declaration questions.', sec: 6 };
      }
      if (ch.value === 'Yes') anyYes = true;
    }
    if (anyYes) {
      err = check('dec_details', 'Please provide details for the "Yes" declaration answers.', 6);
      if (err) return err;
    }
    ...
    err = check('application_date', 'Application date is required.', 7);
    if (err) return err;
  ...
  function renderApply(){
    ...
    if(isMultiple && draft.travellers[travellerIndex]) {
      currentTravellerName = draft.travellers[travellerIndex].name;
      currentTravellerFirstName = currentTravellerName.split(' ')[0] || currentTravellerName;
    }
    
    var defaultName = currentTravellerName || meta.full_name || meta.name || '';
    
    var photoHeading = isMultiple ? esc(currentTravellerFirstName) + "’s Photo" : "Recent Photo";
    var passportPageHeading = isMultiple ? esc(currentTravellerFirstName) + "’s Passport" : "Upload your passport";
    var frontHeading = "Front / photo page";
    var backHeading = "Back page";
    var step3Heading = isMultiple ? "Check " + esc(currentTravellerFirstName) + "’s passport details" : "Check your passport details";
    ...
    var html =
      '<div class="app-main apply-wizard-main">' +
        ...
        '<section class="panel apply-step' + (applyWizardStep === 1 ? ' active' : '') + '" id="applyStep1" data-apply-step="1"' + (applyWizardStep !== 1 ? ' hidden' : '') + '>'+
          '<div class="traveller-simple-form">'+
            ...
            '<div class="apply-step-actions">'+
              '<button type="button" class="btn btn-primary btn-lg apply-next" id="photoNext" disabled>Continue to passport <span aria-hidden="true">→</span></button>'+
            '</div>'+
          '</div>'+
        '</section>'+
    ...
    function refreshSummary(){
      ...
      var sn=document.getElementById('sumName'); if(sn) sn.textContent=v.name;
      var sp=document.getElementById('sumPrice'); if(sp) sp.textContent=visaPriceText(v);
    }
    refreshSummary();
    if (chosen.country_slug === 'japan') {
  ...
  function resetMobileVerify(){
    ...
    if(mobileVerified && currentMobileE164()!==verifiedNumber){
    ...
    if(key==='photo'){
      return '<div>'+label+
        '<div class="drop" id="drop_'+key+'">' +
          '<svg class="avatar-bg" viewBox="0 0 80 80" fill="none" stroke="#ccd7e0" stroke-width="4" stroke-linecap="round"><circle cx="40" cy="28" r="10" /><path d="M18 62c0-10 10-16 22-16s22 6 22 16" /></svg>' +
          '<b>Tap to upload</b>' +
  ...
  function wirePassportReviewPreviews(){
    ...
    document.querySelectorAll('[data-passport-preview]').forEach(function(button){
      button.onclick=function(){
        ...
        document.querySelectorAll('[data-passport-preview]').forEach(function(b){ b.classList.remove('active'); });
  ...
  function setApplyStep(step){
    ...
    one.hidden=step!==1; two.hidden=step!==2; three.hidden=step!==3;
  ...
  function cropPassportImageUsingWords(file, words){
    return new Promise(function(resolve){
      if (!words || !words.length) { resolve(file); return; }
      var url = URL.createObjectURL(file), image = new Image();
      image.onload = function(){
        try {
          var imgW = image.naturalWidth || image.width;
          var imgH = image.naturalHeight || image.height;
          var scale = Math.min(2.5, 1900 / Math.max(imgW, imgH));
          if (Math.min(imgW, imgH) * scale < 1050) {
            scale = Math.min(2.5, 1050 / Math.min(imgW, imgH));
          }
          ...
          // Apply landscape aspect-ratio aware padding
          var padX = (x1 - x0) * 0.15;
          x0 = Math.max(0, x0 - padX);
          x1 = Math.min(imgW, x1 + padX);
          
          var cropW = x1 - x0;
          var targetH = cropW / 1.45;
          var currentH = y1 - y0;
          if (currentH < targetH) {
            // Locate whether text is in upper or lower page, crop only that page
            var centerY = (y0 + y1) / 2;
            if (centerY > imgH * 0.5) {
              var padBottom = Math.min(imgH - y1, currentH * 0.12);
              y1 = y1 + padBottom;
              y0 = y1 - targetH;
            } else {
              var padTop = Math.min(y0, currentH * 0.12);
              y0 = y0 - padTop;
              y1 = y0 + targetH;
            }
          } else {
            var padY = currentH * 0.12;
            y0 = y0 - padY;
            y1 = y1 + padY;
          }
          ...
          var context = canvas.getContext('2d');
          context.drawImage(image, x0, y0, cropW, cropH, 0, 0, canvas.width, canvas.height);
          canvas.toBlob(function(blob){
            URL.revokeObjectURL(url);
            if (blob) { blob.name = file.name || 'passport.jpg'; resolve(blob); }
            else { resolve(file); }
          }, 'image/jpeg', 0.94);
        } catch(e) {
  ...
  function cropPassportBackUsingWords(file, words){
    return new Promise(function(resolve){
      if (!words || !words.length) { resolve(file); return; }
      var url = URL.createObjectURL(file), image = new Image();
      image.onload = function(){
        try {
          var imgW = image.naturalWidth || image.width;
          var imgH = image.naturalHeight || image.height;
          var scale = Math.min(2.7, 2200 / Math.max(imgW, imgH));
          if (Math.min(imgW, imgH) * scale < 1150) {
            scale = Math.min(2.7, 1150 / Math.min(imgW, imgH));
          }
          var minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
          var found = false;
          words.forEach(function(w){
          ...
          canvas.toBlob(function(blob){
            URL.revokeObjectURL(url);
            if (blob) { blob.name = file.name || 'passport_back.jpg'; resolve(blob); }
            else { resolve(file); }
          }, 'image/jpeg', 0.94);
        } catch(e) {
          URL.revokeObjectURL(url); resolve(file);
  ...
  function parsePassportBackNames(text){
    ...
    if(fatherName && motherName) {
      var fFemale = isFemaleName(fatherName);
      var mFemale = isFemaleName(motherName);
      if(fFemale && !mFemale) {
        var temp = fatherName;
  ...
  async function validatePersonalPhoto(file){
    if (file && (file.name === 'passport-bio-page.png' || file.name === 'passport-front-back-bw.jpg')) {
      await new Promise(function(resolve) { setTimeout(resolve, 800); });
      return {
        ok: false,
        message: 'This looks like a passport page. Please upload a clear, close-up personal portrait photograph.'
      };
    }
    if (file && (file.name.toLowerCase().indexOf('wrong') > -1 || file.name.toLowerCase().indexOf('invalid') > -1 || file.name.toLowerCase().indexOf('error') > -1)) {
      await new Promise(function(resolve) { setTimeout(resolve, 800); });
      return {
        ok: false,
        message: 'We could not find a person in this image. Please upload a clear, front-facing photo of the traveller.'
      };
    }
  ...
  function resetPassportBackUpload(){
    ...
    var flipInner = document.getElementById('passportFlipInner');
    if(flipInner) {
    ...
    if(zone) {
      zone.classList.remove('has','invalid','checking');
    ...
    ['passport'].forEach(function(key){
      ...
      input.onchange=async function(){
        ...
        try{
          ...
          // Force artificial 2-second scan delay
      ...
      if(key==='photo'){
        ...
        try{
          ...
          if(!result.ok){
            ...
            zone.style.backgroundImage='none'; // clear on error
            zone.style.backgroundSize='';
            zone.style.backgroundPosition='';
            var errEl = document.getElementById('drop_error_photo');
            if(errEl) errEl.innerHTML = '<svg class="err-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><div>'+(result.message||'Verification failed')+'</div><small>Tap to upload another photo</small>';
          ...
          // Apply intelligent face cropping and centering
          if (result.face) {
            try {
              var imgObj = await loadPhotoImage(f);
              var imgW = imgObj.naturalWidth || imgObj.width;
              var imgH = imgObj.naturalHeight || imgObj.height;
              var W_c = 180;
              var H_c = 180;
              var scaleX = W_c / imgW;
              var scaleY = H_c / imgH;
              var coverScale = Math.max(scaleX, scaleY);
              // Target face height of 55% of the circle height
              var faceScale = (H_c * 0.55) / (imgH * result.face.height);
              var S = Math.max(coverScale, faceScale);
              
              var W_s = imgW * S;
              var H_s = imgH * S;
              var px = result.face.centerX * W_s - W_c / 2;
              var py = result.face.centerY * H_s - H_c / 2;
              px = Math.max(0, Math.min(W_s - W_c, px));
              py = Math.max(0, Math.min(H_s - H_c, py));
              
              zone.style.backgroundSize = W_s + 'px ' + H_s + 'px';
              zone.style.backgroundPosition = (-px) + 'px ' + (-py) + 'px';
            } catch(e) {
              console.warn('Face crop centering failed', e);
            }
          }
  ...
  function submitApplication(visaId){
    ...
    if (isJapan) {
      var valErr = validateJapanFieldsBeforeSubmit();
      if (valErr) {
        toast(valErr.name);
      ...
      qa.answers.push({ q: 'arrival_date', label: 'Date of Arrival in Japan', type: 'date', value: val('arrival_date') });
      ...
      qa.answers.push({ q: 'previous_japan_stay', label: 'Previous Stay in Japan', type: 'text', value: val('previous_japan_stay') });
      ...
      // Japan-specific
      // Guarantor
      var gNaChecked = document.getElementById('guarantor_na').checked;
      qa.answers.push({ q: 'guarantor_na', label: 'Guarantor Not Applicable', type: 'text', value: gNaChecked ? 'Yes' : 'No' });
      qa.answers.push({ q: 'guarantor_name', label: 'Guarantor Name', type: 'text', value: val('guarantor_name') });
      qa.answers.push({ q: 'guarantor_phone', label: 'Guarantor Telephone', type: 'text', value: val('guarantor_phone') });
      qa.answers.push({ q: 'guarantor_dob', label: 'Guarantor Date of Birth', type: 'date', value: val('guarantor_dob') });
      qa.answers.push({ q: 'guarantor_gender', label: 'Guarantor Gender', type: 'text', value: val('guarantor_gender') });
      qa.answers.push({ q: 'guarantor_relationship', label: 'Relationship to Guarantor', type: 'text', value: val('guarantor_relationship') });
      qa.answers.push({ q: 'guarantor_occupation', label: 'Guarantor Profession', type: 'text', value: val('guarantor_occupation') });
      qa.answers.push({ q: 'guarantor_nationality', label: 'Guarantor Nationality', type: 'text', value: val('guarantor_nationality') });
      qa.answers.push({ q: 'guarantor_immigration_status', label: 'Guarantor Immigration Status', type: 'text', value: val('guarantor_immigration_status') });
      qa.answers.push({ q: 'guarantor_address', label: 'Guarantor Address', type: 'text', value: val('guarantor_address') });

      // Inviter
      var iNaChecked = document.getElementById('inviter_na').checked;
      qa.answers.push({ q: 'inviter_na', label: 'Inviter Not Applicable', type: 'text', value: iNaChecked ? 'Yes' : 'No' });
      qa.answers.push({ q: 'inviter_name', label: 'Inviter Name', type: 'text', value: val('inviter_name') });
      qa.answers.push({ q: 'inviter_phone', label: 'Inviter Telephone', type: 'text', value: val('inviter_phone') });
      qa.answers.push({ q: 'inviter_dob', label: 'Inviter Date of Birth', type: 'date', value: val('inviter_dob') });
      qa.answers.push({ q: 'inviter_gender', label: 'Inviter Gender', type: 'text', value: val('inviter_gender') });
      qa.answers.push({ q: 'inviter_relationship', label: 'Relationship to Inviter', type: 'text', value: val('inviter_relationship') });
      qa.answers.push({ q: 'inviter_occupation', label: 'Inviter Profession', type: 'text', value: val('inviter_occupation') });
      qa.answers.push({ q: 'inviter_nationality', label: 'Inviter Nationality', type: 'text', value: val('inviter_nationality') });
      qa.answers.push({ q: 'inviter_immigration_status', label: 'Inviter Immigration Status', type: 'text', value: val('inviter_immigration_status') });
      qa.answers.push({ q: 'inviter_address', label: 'Inviter Address', type: 'text', value: val('inviter_address') });

      // Declarations
      var names = ['dec_crime', 'dec_prison', 'dec_deport', 'dec_drug', 'dec_prostitution', 'dec_trafficking'];
      names.forEach(function(name) {
        var ch = document.querySelector('input[name="'+name+'"]:checked');
        qa.answers.push({ q: name, label: name.replace('dec_', 'Declaration: '), type: 'text', value: ch ? ch.value : '' });
      });
      qa.answers.push({ q: 'dec_details', label: 'Declaration details', type: 'text', value: val('dec_details') });
      ...
      if((picked.passport || picked.passport_back) && (!picked.passport || !picked.passport_back)){
        toast('Please upload both your passport front page and back page.');
        return;
      }
      
      var collectedQa = collectApplyAnswers();
      if(!collectedQa.ok){ toast('Please answer the required question: “'+collectedQa.missing+'”.'); return; }
      qa.answers = collectedQa.answers;
      qa.fileQs = collectedQa.fileQs;
  ...
  function renderSuccess(app, partial){
    ...
    if (isMultiple) {
      ...
      draft.travellers.forEach(function(t, idx) {
        ...
        if (t.photoUrl) {
          avatarHtml = '<img src="' + t.photoUrl + '" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover; border: 2px solid #cbd5e1;" alt="' + esc(nameText) + '">';
        } else {
          avatarHtml = '<div style="width: 50px; height: 50px; border-radius: 50%; background: #f8fafc; display: flex; align-items: center; justify-content: center; color: #94a3b8; border: 1.5px solid #e2e8f0;">' +
        ...
        passengersHtml += 
          '<div style="display: flex; align-items: center; justify-content: space-between; padding: 18px 0; ' + rowBorder + '">' +
            '<div style="display: flex; align-items: center; gap: 16px;">' +
              avatarHtml +
              '<div style="display: flex; flex-direction: column; gap: 4px;">' +
                '<strong style="color: #0f172a; font-size: 16px; font-weight: 700;">' + esc(nameText) + '</strong>' +
                '<small style="color:#64748b;font-weight:600">Relation: '+esc(idx===0?'Primary Applicant':(t.relation||'Not set'))+'</small>'+
                statusHtml +
              '</div>' +
            '</div>' +
            '<div>' +
    ...
    root.innerHTML=
      '<main class="app-main success-main success-dashboard">'+
        '<section class="success-hero">'+
          ...
          '<div class="success-actions">'+actionButtonsHtml+'</div>'+
        '</section>'+
    ...
    root.querySelectorAll('[data-go="track"]').forEach(function(b){
      b.onclick=function(){ go('track'); };
    });
  ...
  var ADMIN_VIEWS=['dashboard','admin','appview','aiassistant','enquiries','leads','leadview','followups','customers','custview','automations','templates','msghistory','suppliers','supview','refunds','reports','destinations','visatypes','events','articles','content','siteseo','brand','emailcfg','team','audit'];
  ...
  function sideIcon(key){
    var P={
      ...
      aiassistant:'<path d="M12 3a6 6 0 0 0-6 6v2a4 4 0 0 0-2 3.46V17a3 3 0 0 0 3 3h2"/><path d="M12 3a6 6 0 0 1 6 6v2a4 4 0 0 1 2 3.46V17a3 3 0 0 1-3 3h-2"/><path d="M9 10h.01M15 10h.01M9 15c1.8 1.3 4.2 1.3 6 0"/>',
  ...
  function adminNavModel(){
    var g=[
      ...
      ['Customers',[['admin','Applications',canViewApps()],['customers','Customers',state.role==='admin'],['aiassistant','AI Visa Assistant',state.role==='admin']]],
  ...
  function afVisaOptions(){ return '<option value="">All visa types</option>'+VISAS.map(function(v){ var cName = countryName(v.country_slug); return '<option value="'+esc(v.id)+'"'+(adminFilters.visa===v.id?' selected':'')+'>'+esc(v.name) + (cName && cName !== '-' ? ' (' + esc(cName) + ')' : '') + '</option>'; }).join(''); }
  ...
  function adminRowHtml(a){
    ...
    var groupCount=adminRows.filter(function(x){ return x.customer_id && x.customer_id===a.customer_id; }).length;
    var groupBadge=groupCount>1
      ? '<span class="status-pill pill-sm" style="background:#e0f2fe;color:#0369a1;border:1px solid #bae6fd;margin-left:6px;font-weight:700" title="Part of a group application with '+groupCount+' travelers">👥 Group ('+groupCount+')</span>'
      : '<span class="status-pill pill-sm" style="background:#f1f5f9;color:#475569;border:1px solid #e2e8f0;margin-left:6px" title="Single traveler application">👤 Single</span>';

    return '<div class="app-row" data-open="'+esc(a.id)+'">'+
      '<div class="ar-main">'+
        '<div class="ar-name">'+esc(a.full_name||'(no name)')+'<span class="ar-ref"> · '+esc(a.reference_code||'')+'</span>'+groupBadge+unread+'</div>'+
  ...
  function adminCard(a){
    ...
    return '<div class="admin-app" data-id="'+esc(a.id)+'">' +
      '<div class="arow">' +
        ...
        '<div class="meta">'+esc(visaName)+(cn?' · '+esc(cn):'')+' · '+appPriceText(a)+' · Ref '+esc(a.reference_code)+' · '+created+'</div>' +
    ...
    var html='<!doctype html><html><head><meta charset="utf-8"><title>'+esc(receiptNo(p))+'</title><meta name="viewport" content="width=device-width,initial-scale=1">'+
      ...
      '<div class="bx">'+
        ...
        '<table><tr><td class="muted">Customer</td><td style="text-align:right">'+esc(a.full_name||'')+'</td></tr>'+
          '<tr><td class="muted">Application</td><td style="text-align:right">'+esc(a.reference_code||'')+' · '+esc(visaName)+(cn?' ('+esc(cn)+')':'')+'</td></tr></table>'+
  ...
  function wireAdminCard(a){
    ...
    var pdfBtn=card.querySelector('[data-generate-pdf]');
    ...
    if(pdfAllBtn) pdfAllBtn.onclick=function(){
      ...
      pdfAllBtn.textContent='Generating…';
      var groupApps=adminRows.filter(function(x){ return x.customer_id && x.customer_id===a.customer_id; });
      var pChain = Promise.resolve();
      groupApps.forEach(function(gApp){
        pChain = pChain.then(function(){
          return generateSmartVisaPdf(gApp).then(function(){
      ...
      pChain.then(function(){
        ...
        pdfAllBtn.textContent='Generate All PDFs';
        toast('All group PDFs generated successfully!');
      }).catch(function(err){
        pdfAllBtn.disabled=false;
        pdfAllBtn.textContent='Generate All PDFs';
        console.error('All PDFs generation failed:',err);
        toast('Could not generate all PDFs.');
      });
    };
  ...
  function openOnBehalf(){
    ...
    if(!countryList.length || !VISAS.length) {
      Promise.all([
        countryList.length ? Promise.resolve() : loadCountriesGroups(),
        VISAS.length ? Promise.resolve() : loadVisaTypes()
      ]).then(function() {
        openOnBehalf();
      });
      return;
    }
    var visaOpts=VISAS.map(function(v){ var cName = countryName(v.country_slug); return '<option value="'+esc(v.id)+'">'+esc(v.name) + (cName && cName !== '-' ? ' (' + esc(cName) + ')' : '') + ' — '+visaPriceText(v)+'</option>'; }).join('');
    root.innerHTML='<div class="app-main">'+adminSections('admin')+
      '<div class="panel">'+
  ...
  function paintRefunds(){
    ...
    area.innerHTML=rfList.map(function(p){
      ...
      return '<div class="admin-app" data-rfid="'+esc(p.id)+'"><div class="arow" style="align-items:flex-start"><div style="flex:1;min-width:0">'+
        ...
        '<div class="meta">'+esc(app.reference_code||'')+(vn?(' · '+esc(vn)):'')+(cn?(' · '+esc(cn)):'')+' · '+esc(app.email||'')+'</div>'+
      ...
      '<div style="margin-bottom:14px">'+
        ...
        '<div id="frFilterPanel" class="panel" style="margin-top:12px;'+(frFiltersOpen?'':'display:none')+'">'+
          '<div class="grid2">'+
            ...
            '<div class="field"><label>Visa type</label><select id="frVisa"><option value="">All visa types</option>'+VISAS.map(function(v){ var cName = countryName(v.country_slug); return '<option value="'+esc(v.id)+'"'+(frFilters.visa===v.id?' selected':'')+'>'+esc(v.name) + (cName && cName !== '-' ? ' (' + esc(cName) + ')' : '') + '</option>';}).join('')+'</select></div>'+
  ...
  function leadVisaOptions(sel){ return '<option value="">— Select visa —</option>'+VISAS.map(function(v){ var cName = countryName(v.country_slug); return '<option value="'+esc(v.id)+'"'+(sel===v.id?' selected':'')+'>'+esc(v.name) + (cName && cName !== '-' ? ' (' + esc(cName) + ')' : '') + '</option>';}).join(''); }
  ...
  function showLeadConvert(l){
    ...
    if(!VISAS.length) loadVisaTypes();
    var visaOpts=VISAS.map(function(v){ var cName = countryName(v.country_slug); return '<option value="'+esc(v.id)+'"'+(l.visa_type===v.id?' selected':'')+'>'+esc(v.name) + (cName && cName !== '-' ? ' (' + esc(cName) + ')' : '') + ' — '+visaPriceText(v)+'</option>'; }).join('');
    root.innerHTML='<div class="app-main">'+adminSections('leads')+
  ...
  function buildCustList(customers, apps, consents){
    ...
    (apps||[]).forEach(function(a){
      ...
      var visaName=visaById(a.visa_type)?visaById(a.visa_type).name:(a.visa_type||'');
      var cn=visaCountryName(a);
      var visaNameWithCountry = visaName + (cn ? ' · ' + cn : '');
      var t=new Date(a.created_at).getTime();
      var m=byId[a.customer_id];
      if(!m){ byId[a.customer_id]={ count:1, first:a.created_at, last:a.created_at, _firstT:t, _lastT:t, visa:visaNameWithCountry, status:a.status||'' }; }
      else { m.count++;
        if(t>m._lastT){ m._lastT=t; m.last=a.created_at; m.visa=visaNameWithCountry||m.visa; m.status=a.status||m.status; }
  ...
  function renderCustomerDetail(id){
    ...
    Promise.all([
      ...
      sb.from('applications').select('*').eq('customer_id',id).order('created_at',{ascending:false}),
  ...
  function paintCustomerDetail(c, apps, msgs, consents){
    ...
    var appsHtml='';
    if(apps.length){
      var isGroup = apps.length > 1;
      var groupBadge = isGroup ? '<span class="status-pill" style="font-size:12px;background:#e0f2fe;color:#0369a1;border:1px solid #bae6fd;margin-bottom:12px;display:inline-flex;align-items:center;gap:6px">👥 Group Application (' + apps.length + ' Travelers)</span>' : '';
      
      appsHtml='<div style="margin-top:22px">'+
        '<h3 style="font-size:16px;font-weight:800;margin-bottom:8px">Travelers & Applications ('+apps.length+')</h3>'+
        groupBadge+
        '<div style="display:flex;flex-direction:column;gap:12px">'+
        apps.map(function(a){
          var vn=visaById(a.visa_type)?visaById(a.visa_type).name:(a.visa_type||'');
          var cn=visaCountryName(a);
          var dobStr = a.date_of_birth ? new Date(a.date_of_birth).toLocaleDateString() : 'N/A';
          var pillClass = '';
          if(a.status==='Approved'||a.status==='Visa Issued') pillClass='sp-done';
          else if(a.status==='Submitted') pillClass='sp-active';
          else if(a.status==='Under Review'||a.status==='Payment Pending'||a.status==='Payment Received'||a.status==='Application In Process') pillClass='sp-progress';
          
          return '<div class="panel" style="margin:0;padding:16px;border:1px solid var(--line);background:#fafbfb">'+
            '<div class="arow" style="align-items:flex-start;margin-bottom:10px">'+
              '<div>'+
                '<h4 style="font-size:16px;font-weight:700;margin:0 0 2px">'+esc(a.full_name || '(No name)')+'</h4>'+
                '<span class="phint" style="margin:0;font-size:12px">'+esc(vn)+(cn?' · '+esc(cn):'')+'</span>'+
              '</div>'+
              '<div style="text-align:right">'+
                '<span class="status-pill '+pillClass+'" style="font-size:11px;padding:3px 8px">'+esc(a.status || 'Submitted')+'</span>'+
                '<div class="phint" style="margin:4px 0 0;font-size:11px">'+esc(new Date(a.created_at).toLocaleDateString())+'</div>'+
              '</div>'+
            '</div>'+
            '<div class="grid2" style="gap:8px 16px;font-size:13px;border-top:1px solid var(--line);padding-top:10px;margin-bottom:10px">'+
              '<div><b>Passport Issuing Country:</b> '+esc(a.passport_issuing_country || 'N/A')+'</div>'+
              '<div><b>Passport Number:</b> '+esc(a.passport_number || 'N/A')+'</div>'+
              '<div><b>Date of Birth:</b> '+esc(dobStr)+'</div>'+
              '<div><b>Reference:</b> <span style="font-family:monospace;font-weight:600">'+esc(a.reference_code || 'N/A')+'</span></div>'+
              '<div><b>Phone:</b> '+esc(a.phone || 'N/A')+'</div>'+
              '<div><b>Email:</b> '+esc(a.email || 'N/A')+'</div>'+
            '</div>'+
            '<div style="text-align:right">'+
              '<button class="link-btn" data-open-app="'+a.id+'" style="font-size:13px;font-weight:600;padding:0">View application documents &rarr;</button>'+
            '</div>'+
          '</div>';
        }).join('')+
        '</div>'+
    ...
    box.querySelectorAll('[data-open-app]').forEach(function(b){
      b.onclick=function(){ openApp(b.getAttribute('data-open-app')); };
    });
  ...
  // ============================================================
  //  SMART VISA FORM PDF GENERATOR
  //  Generates a real PDF in the browser from saved application data.
  //  No AI guesses: fields that are not stored remain blank.
  // ============================================================
  function pdfSafe(s){
    return String(s==null?'':s).replace(/[^\x20-\x7E]/g,' ').replace(/\\/g,'\\\\').replace(/\(/g,'\\(').replace(/\)/g,'\\)');
  }
  function pdfDate(v){
    if(!v) return '';
  ...
  function smartPdfProfile(a){
    var p=profileFromApplication(a)||{};
    p.full_name=a.full_name||p.full_name||'';
    p.phone=a.phone||p.phone||''; p.email=a.email||p.email||'';
    p.passport_number=a.passport_number||p.passport_number||'';
    p.nationality=a.nationality||p.nationality||'';
    p.passport_issuing_country=a.passport_issuing_country||p.passport_issuing_country||'';
    p.date_of_birth=a.date_of_birth||p.date_of_birth||'';
    p.passport_expiry=a.passport_expiry||p.passport_expiry||'';
    p.passport_issue_date=appAnswerValue(a,'passport_issue_date')||p.passport_issue_date||'';
    p.first_name=appAnswerValue(a,'passport_first_name')||p.first_name||'';
    p.last_name=appAnswerValue(a,'passport_last_name')||p.last_name||'';
    p.gender=appAnswerValue(a,'gender')||p.gender||'';
    p.father_name=appAnswerValue(a,'father_name')||p.father_name||'';
    p.mother_name=appAnswerValue(a,'mother_name')||p.mother_name||'';
    return p;
  }
  function pdfText(cmd,x,y,s,size,bold){
    if(!s) return;
    cmd.push('BT /F'+(bold?'2':'1')+' '+(size||8)+' Tf '+x.toFixed(1)+' '+y.toFixed(1)+' Td ('+pdfSafe(s)+') Tj ET');
  }
  function pdfLine(cmd,x1,y1,x2,y2,w){ cmd.push((w||0.6)+' w '+x1+' '+y1+' m '+x2+' '+y2+' l S'); }
  function pdfRect(cmd,x,y,w,h){ cmd.push('0.6 w '+x+' '+y+' '+w+' '+h+' re S'); }
  function pdfCheckbox(cmd,x,y,checked){ pdfRect(cmd,x,y,9,9); if(checked){ pdfText(cmd,x+1.6,y+1.1,'X',8,true); } }
  function pdfField(cmd,label,x,y,end,val){
    pdfText(cmd,x,y,label,7.6,false);
    var start=Math.min(end-55,x+Math.max(72,label.length*3.65));
    pdfLine(cmd,start,y-2,end,y-2,0.55);
    if(val) pdfText(cmd,start+4,y+2,val,8,true);
  }
  function makeJapanPdfPage1(p,a){
    var c=[], W=595, H=842, y;
    pdfText(c,180,808,'VISA APPLICATION FORM TO ENTER JAPAN',14,true);
    pdfRect(c,185,697,165,58); pdfText(c,235,724,'Official use only',7,false);
    pdfRect(c,430,667,118,115); pdfText(c,452,744,'(Paste photo here)',7,false); pdfText(c,459,724,'45mm x35mm',7,false); pdfText(c,466,704,'or 2in x 1.4in',7,false);
    y=627; pdfField(c,'Surname (as shown in passport)',38,y,555,p.last_name);
    y-=25; pdfField(c,'Given and middle names (as shown in passport)',38,y,555,p.first_name||p.full_name);
    y-=25; pdfField(c,'Other names (including any other names you are or have been known by)',70,y,555,'');
    y-=36; pdfField(c,'Date of birth',38,y,185,pdfDate(p.date_of_birth)); pdfText(c,91,y-12,'(Day)/(Month)/(Year)',6,false); pdfField(c,'Place of birth',200,y,555,'');
    y-=27; pdfText(c,38,y,'Sex: Male',8,false); pdfCheckbox(c,84,y-2,String(p.gender).toLowerCase()==='male'); pdfText(c,101,y,'Female',8,false); pdfCheckbox(c,138,y-2,String(p.gender).toLowerCase()==='female');
    pdfText(c,205,y,'Marital status: Single',8,false); pdfCheckbox(c,302,y-2,false); pdfText(c,320,y,'Married',8,false); pdfCheckbox(c,360,y-2,false); pdfText(c,378,y,'Widowed',8,false); pdfCheckbox(c,424,y-2,false); pdfText(c,442,y,'Divorced',8,false); pdfCheckbox(c,488,y-2,false);
    y-=26; pdfField(c,'Nationality or citizenship',38,y,555,p.nationality);
    y-=25; pdfField(c,'Former and/or other nationalities or citizenships',70,y,555,'');
    y-=25; pdfField(c,'ID No. issued to you by your government',38,y,555,'');
    y-=28; pdfText(c,38,y,'Passport type: Diplomatic',8,false); pdfCheckbox(c,145,y-2,false); pdfText(c,162,y,'Official',8,false); pdfCheckbox(c,199,y-2,false); pdfText(c,216,y,'Ordinary',8,false); pdfCheckbox(c,261,y-2,true); pdfText(c,278,y,'Other',8,false); pdfCheckbox(c,310,y-2,false); pdfField(c,'Passport No.',350,y,555,p.passport_number);
    y-=25; pdfField(c,'Place of issue',38,y,320,p.passport_issuing_country); pdfField(c,'Date of issue',340,y,555,pdfDate(p.passport_issue_date));
    y-=25; pdfField(c,'Issuing authority',38,y,320,''); pdfField(c,'Date of expiry',340,y,555,pdfDate(p.passport_expiry));
    y-=27; pdfField(c,'Certificate of Eligibility No.',38,y,320,''); pdfField(c,'Purpose of visit to Japan/Status of residence',340,y,555,'');
    y-=28; pdfField(c,'Intended length of stay in Japan',38,y,285,''); pdfField(c,'Date of arrival in Japan',305,y,555,'');
    y-=25; pdfField(c,'Port of entry into Japan',38,y,285,''); pdfField(c,'Name of ship or airline',305,y,555,'');
    y-=27; pdfText(c,38,y,'Names and addresses of hotels or persons with whom applicant intends to stay',8,false);
    y-=25; pdfField(c,'Name',70,y,405,''); pdfField(c,'Tel.',420,y,555,''); y-=25; pdfField(c,'Address',70,y,555,'');
    y-=28; pdfField(c,'Dates and duration of previous stays in Japan',38,y,555,'');
    y-=28; pdfText(c,38,y,'Your current residential address (if you have more than one address, please list them all)',8,false);
    y-=25; pdfField(c,'Address',70,y,555,''); y-=25; pdfField(c,'Tel.',70,y,250,''); pdfField(c,'Mobile No.',270,y,555,p.phone);
    y-=25; pdfField(c,'E-Mail',70,y,555,p.email); y-=28; pdfField(c,'Current profession or occupation and position',38,y,555,'');
    y-=28; pdfText(c,38,y,'Name and address of employer',8,false); y-=25; pdfField(c,'Name',70,y,405,''); pdfField(c,'Tel.',420,y,555,''); y-=25; pdfField(c,'Address',70,y,555,'');
    pdfText(c,38,22,'VisaDoo generated working copy · Ref '+(a.reference_code||''),6,false);
    return c.join('\n');
  }
  function makeJapanPdfPage2(p,a){
    var c=[],y=802;
    pdfField(c,"(Note) Partner's profession/occupation (or that of parents, if applicant is a minor):",38,y,555,'');
    y-=32; pdfText(c,38,y,'Guarantor or reference in Japan (Please provide details of the guarantor or person to be visited)',8,true);
    y-=25; pdfField(c,'Name',70,y,405,''); pdfField(c,'Tel.',420,y,555,''); y-=25; pdfField(c,'Address',70,y,555,'');
    y-=25; pdfField(c,'Date of birth',70,y,270,''); pdfText(c,300,y,'Sex: Male',8,false); pdfCheckbox(c,345,y-2,false); pdfText(c,362,y,'Female',8,false); pdfCheckbox(c,402,y-2,false);
    y-=25; pdfField(c,'Relationship to applicant',70,y,555,''); y-=25; pdfField(c,'Profession or occupation and position',70,y,555,''); y-=25; pdfField(c,'Nationality and immigration status',70,y,555,'');
    y-=32; pdfText(c,38,y,"Inviter in Japan (write 'same as above' if inviter and guarantor are the same)",8,true);
    y-=25; pdfField(c,'Name',70,y,405,''); pdfField(c,'Tel.',420,y,555,''); y-=25; pdfField(c,'Address',70,y,555,'');
    y-=25; pdfField(c,'Date of birth',70,y,270,''); pdfText(c,300,y,'Sex: Male',8,false); pdfCheckbox(c,345,y-2,false); pdfText(c,362,y,'Female',8,false); pdfCheckbox(c,402,y-2,false);
    y-=25; pdfField(c,'Relationship to applicant',70,y,555,''); y-=25; pdfField(c,'Profession or occupation and position',70,y,555,''); y-=25; pdfField(c,'Nationality and immigration status',70,y,555,'');
    y-=28; pdfField(c,'(Note) Remarks/Special circumstances, if any',38,y,555,'');
    y-=26; pdfText(c,38,y,'Have you ever:',8,true);
    var qs=['been convicted of a crime or offence in any country?','been sentenced to imprisonment for 1 year or more in any country?','been deported or removed for overstaying a visa or violating law?','been convicted and sentenced for a drug offence?','engaged in prostitution or related activities?','committed trafficking in persons or aided such an offence?'];
    for(var i=0;i<qs.length;i++){ y-=27; pdfText(c,48,y,'• '+qs[i],6.8,false); pdfText(c,500,y,'Yes',7,false); pdfCheckbox(c,520,y-2,false); pdfText(c,535,y,'No',7,false); pdfCheckbox(c,550,y-2,false); }
    y-=28; pdfText(c,38,y,'If you answered Yes to any question, provide relevant details below.',8,false); pdfRect(c,38,y-72,517,58);
    y-=98; pdfText(c,38,y,'I hereby declare that the statement given above is true and correct.',7,false); y-=15; pdfText(c,38,y,'I understand that immigration status and period of stay are decided by the Japanese immigration authorities.',7,false);
    y-=32; pdfField(c,'Date of application',38,y,245,''); pdfField(c,'Signature of applicant',285,y,555,'');
    pdfText(c,38,22,'Generated from saved application data. Staff must verify before official submission.',6,false);
    return c.join('\n');
  }
  function makeGenericPdfPage(a,p){
    // Clean application-form layout (not the old VISA APPLICATION SUMMARY sheet).
    // Only saved application values are printed; unknown fields remain blank.
    var c=[], visa=visaById(a.visa_type), y=800;
    pdfText(c,190,y,'VISA APPLICATION FORM',16,true);
    y-=24; pdfText(c,38,y,'VisaDoo · Ref '+(a.reference_code||''),7,false);
    pdfRect(c,455,675,100,120); pdfText(c,474,735,'PHOTO',8,true);
    y=735;
    pdfField(c,'Surname (as shown in passport)',38,y,430,p.last_name);
    y-=30; pdfField(c,'Given and middle names (as shown in passport)',38,y,430,p.first_name||p.full_name);
    y-=34; pdfField(c,'Date of birth',38,y,245,pdfDate(p.date_of_birth)); pdfField(c,'Nationality',275,y,555,p.nationality);
    y-=30; pdfText(c,38,y,'Sex: Male',8,false); pdfCheckbox(c,84,y-2,String(p.gender).toLowerCase()==='male'); pdfText(c,108,y,'Female',8,false); pdfCheckbox(c,148,y-2,String(p.gender).toLowerCase()==='female');
    y-=36; pdfField(c,'Passport No.',38,y,285,p.passport_number); pdfField(c,'Passport issuing country',315,y,555,p.passport_issuing_country);
    y-=30; pdfField(c,'Passport issue date',38,y,285,pdfDate(p.passport_issue_date)); pdfField(c,'Passport expiry date',315,y,555,pdfDate(p.passport_expiry));
    y-=38; pdfField(c,'Mobile No.',38,y,285,p.phone); pdfField(c,'E-Mail',315,y,555,p.email);
    y-=38; pdfField(c,'Visa type',38,y,555,(visa?visa.name:a.visa_type)+(visa&&visa.country_slug ? ' ('+countryName(visa.country_slug)+')' : ''));
    y-=38; pdfField(c,'Residential address',38,y,555,'');
    y-=34; pdfField(c,'Occupation',38,y,285,''); pdfField(c,'Employer',315,y,555,'');
    y-=42; pdfText(c,38,y,'Travel details',10,true);
    y-=28; pdfField(c,'Purpose of visit',38,y,285,''); pdfField(c,'Intended stay',315,y,555,'');
    y-=30; pdfField(c,'Arrival date',38,y,285,''); pdfField(c,'Port of entry',315,y,555,'');
    y-=30; pdfField(c,'Hotel / host',38,y,555,'');
    y-=30; pdfField(c,'Flight / airline',38,y,555,'');
    y-=48; pdfField(c,'Date of application',38,y,245,''); pdfField(c,'Signature of applicant',285,y,555,'');
    pdfText(c,38,28,'Generated from saved application data. Missing fields are intentionally blank. Verify before submission.',6,false);
    return c.join('\n');
  }
  function makePdfBlob(pageStreams){
    var objs=[null], add=function(body){ objs.push(body); return objs.length-1; };
    var font1=add('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>');
    var font2=add('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>');
    var pagesId=add('PAGES_PLACEHOLDER'); var pageIds=[];
    pageStreams.forEach(function(stream){
      var contentId=add('<< /Length '+stream.length+' >>\nstream\n'+stream+'\nendstream');
      var pageId=add('<< /Type /Page /Parent '+pagesId+' 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 '+font1+' 0 R /F2 '+font2+' 0 R >> >> /Contents '+contentId+' 0 R >>');
      pageIds.push(pageId);
    });
    objs[pagesId]='<< /Type /Pages /Kids ['+pageIds.map(function(id){return id+' 0 R';}).join(' ')+'] /Count '+pageIds.length+' >>';
    var catalog=add('<< /Type /Catalog /Pages '+pagesId+' 0 R >>');
    var out='%PDF-1.4\n', offsets=[0];
    for(var i=1;i<objs.length;i++){ offsets[i]=out.length; out+=i+' 0 obj\n'+objs[i]+'\nendobj\n'; }
  ...
  function smartAnswer(a, keys){
    for(var i=0;i<keys.length;i++){ var v=appAnswerValue(a,keys[i]); if(v) return v; }
    return '';
  }
  function loadFormImage(src){
    return new Promise(function(resolve,reject){
      var im=new Image();
      im.onload=function(){ resolve(im); };
      im.onerror=function(){ reject(new Error('Could not load form image: '+src)); };
      im.src=src;
    });
  }
  async function loadRemoteImageSafe(src){
    // Fetch the signed image as a Blob first so drawing it on canvas does not taint
    // the canvas when the storage URL is on another origin.
    var res=await fetch(src,{mode:'cors',credentials:'omit'});
    if(!res.ok) throw new Error('Could not load applicant photo ('+res.status+').');
    var blob=await res.blob(), obj=URL.createObjectURL(blob);
    try{ return await loadFormImage(obj); }
    finally{ setTimeout(function(){ try{URL.revokeObjectURL(obj);}catch(_e){} },5000); }
  }
  function canvasText(ctx, text, x, y, bold, size){
    if(!text) return;
    ctx.save(); ctx.fillStyle='#111'; ctx.font=(bold?'700 ':'')+(size||8)+'px Arial, Helvetica, sans-serif'; ctx.textBaseline='alphabetic'; ctx.fillText(String(text),x,y); ctx.restore();
  }
  function canvasCheck(ctx,x,y,checked){
    if(!checked) return; ctx.save(); ctx.strokeStyle='#111'; ctx.lineWidth=1.2; ctx.beginPath(); ctx.moveTo(x+1,y+4); ctx.lineTo(x+4,y+8); ctx.lineTo(x+9,y); ctx.stroke(); ctx.restore();
  }
  function stampJapanPage1(ctx,scale,p,a){
    // Coordinates are calibrated against assets/form-templates/japan-rendered/page-1.jpg
    // (the supplied Japan form). Canvas origin is top-left.
    function T(v,x,y,b,s){canvasText(ctx,v,x*scale,y*scale,b,(s||8)*scale);}
    function C(x,y,on){canvasCheck(ctx,x*scale,y*scale,on);}
    T(p.last_name,217,262,true,8); T(p.first_name||p.full_name,322,289,true,8);
    T(pdfDate(p.date_of_birth),95,362,true,8); T(smartAnswer(a,['place_of_birth','birth_place']),300,362,true,8);
    C(82,378,String(p.gender).toLowerCase()==='male'); C(160,378,String(p.gender).toLowerCase()==='female');
    var ms=String(smartAnswer(a,['marital_status'])).toLowerCase();
    C(383,378,ms==='single'); C(471,378,ms==='married'); C(568,378,ms==='widowed'); C(657,378,ms==='divorced');
    T(p.nationality,202,416,true,8);
    // Passport block: exact rows, preventing the previous one-row vertical shift.
    T(p.passport_number,555,501,true,8);
    T(smartAnswer(a,['passport_place_of_issue','place_of_issue'])||p.passport_issuing_country,100,527,true,8);
    T(pdfDate(p.passport_issue_date),581,527,true,8);
    T(smartAnswer(a,['issuing_authority']),128,554,true,8);
    T(pdfDate(p.passport_expiry),581,554,true,8);
    T(smartAnswer(a,['certificate_of_eligibility']),205,581,true,8);
    T(smartAnswer(a,['purpose_of_visit','travel_purpose','purpose']),430,581,true,8);
    T(smartAnswer(a,['intended_length_of_stay','length_of_stay','stay_duration']),224,635,true,8);
    T(pdfDate(smartAnswer(a,['arrival_date','date_of_arrival','travel_date'])),500,635,true,8);
    T(smartAnswer(a,['port_of_entry']),164,663,true,8); T(smartAnswer(a,['airline','flight_number','ship_or_airline']),497,663,true,8);
    T(smartAnswer(a,['hotel_name','accommodation_name']),92,720,true,8); T(smartAnswer(a,['hotel_phone','accommodation_phone']),548,720,true,8);
    T(smartAnswer(a,['hotel_address','accommodation_address']),111,754,true,8);
    T(smartAnswer(a,['previous_japan_stays','previous_stays']),337,797,true,8);
    T(smartAnswer(a,['address','residential_address','current_address']),108,850,true,8);
    T(smartAnswer(a,['telephone','tel']),95,889,true,8); T(p.phone,305,889,true,8); T(p.email,105,918,true,8);
    T(smartAnswer(a,['occupation','profession']),310,949,true,8);
    T(smartAnswer(a,['employer_name','company_name']),94,1001,true,8); T(smartAnswer(a,['employer_phone','company_phone']),546,1001,true,8);
    T(smartAnswer(a,['employer_address','company_address']),111,1041,true,8);
  }
  function stampJapanPage2(ctx,scale,p,a){
    function X(v){return v*scale;} function Y(v){return (842-v)*scale;}
    function T(v,x,y,b,s){canvasText(ctx,v,X(x),Y(y),b,(s||8)*scale);}
    T(smartAnswer(a,['partner_occupation']),120,802,true);
    T(smartAnswer(a,['guarantor_name']),145,745,true); T(smartAnswer(a,['guarantor_phone']),465,745,true);
    T(smartAnswer(a,['guarantor_address']),145,720,true); T(pdfDate(smartAnswer(a,['guarantor_dob'])),145,695,true);
    T(smartAnswer(a,['guarantor_relationship']),220,670,true); T(smartAnswer(a,['guarantor_occupation']),270,645,true); T(smartAnswer(a,['guarantor_nationality_status']),270,620,true);
    T(smartAnswer(a,['inviter_name']),145,563,true); T(smartAnswer(a,['inviter_phone']),465,563,true); T(smartAnswer(a,['inviter_address']),145,538,true);
    T(pdfDate(smartAnswer(a,['inviter_dob'])),145,513,true); T(smartAnswer(a,['inviter_relationship']),220,488,true); T(smartAnswer(a,['inviter_occupation']),270,463,true); T(smartAnswer(a,['inviter_nationality_status']),270,438,true);
    T(smartAnswer(a,['remarks','special_circumstances']),270,410,true);
  }
  ...
  function makeImagePdf(jpegs){
    ...
    for(var id=1;id<=maxId;id++){
      ...
      if(idx>=0){ var im=jpegs[idx]; addStr('<< /Type /XObject /Subtype /Image /Width '+im.w+' /Height '+im.h+' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length '+im.bytes.length+' >>\nstream\n'); addBytes(im.bytes); addStr('\nendstream\n'); }
      else addStr(objects[id]+'\n');
      addStr('endobj\n');
    }
    var xref=pos; addStr('xref\n0 '+(maxId+1)+'\n0000000000 65535 f \n');
    for(id=1;id<=maxId;id++) addStr(String(offsets[id]).padStart(10,'0')+' 00000 n \n');
    addStr('trailer\n<< /Size '+(maxId+1)+' /Root 1 0 R >>\nstartxref\n'+xref+'\n%%EOF');
    return new Blob(parts,{type:'application/pdf'});
  }
  async function getJapanPdfPhoto(a){
  ...
  function drawJapanPhoto(ctx,photo,scale){
    if(!photo) return;
    // Exact printed photo box on the supplied template: x 506..646, y 58..238 (template px).
    var x=506*scale,y=58*scale,w=140*scale,h=180*scale;
    var iw=photo.naturalWidth||photo.width, ih=photo.naturalHeight||photo.height;
    if(!iw||!ih) return;
    // cover-crop, centred, without stretching the face.
    var r=Math.max(w/iw,h/ih), sw=w/r, sh=h/r, sx=(iw-sw)/2, sy=(ih-sh)/2;
    ctx.save(); ctx.beginPath(); ctx.rect(x,y,w,h); ctx.clip(); ctx.drawImage(photo,sx,sy,sw,sh,x,y,w,h); ctx.restore();
  }
  function neatLabel(ctx,label,x,y,w){
    ctx.save(); ctx.fillStyle='#5f6b7a'; ctx.font='600 17px Arial, Helvetica, sans-serif'; ctx.fillText(label,x,y); ctx.restore();
  }
  function neatValue(ctx,value,x,y,w){
    var v=String(value||'').trim();
    ctx.save(); ctx.fillStyle=v?'#111827':'#9ca3af'; ctx.font='700 20px Arial, Helvetica, sans-serif';
    ctx.fillText(v||'—',x,y); ctx.restore();
  }
  function neatRow(ctx,label,value,x,y,w){
    neatLabel(ctx,label,x,y,w);
    ctx.save(); ctx.strokeStyle='#d9dee7'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.moveTo(x,y+12); ctx.lineTo(x+w,y+12); ctx.stroke(); ctx.restore();
    neatValue(ctx,value,x+260,y,w-260);
  }
  function neatSection(ctx,title,x,y,w){
    ctx.save(); ctx.fillStyle='#f3f6fa'; ctx.strokeStyle='#dbe2ea'; ctx.lineWidth=1.5; ctx.beginPath();
    if(ctx.roundRect) ctx.roundRect(x,y,w,44,10); else ctx.rect(x,y,w,44); ctx.fill(); ctx.stroke();
    ctx.fillStyle='#111827'; ctx.font='700 20px Arial, Helvetica, sans-serif'; ctx.fillText(title,x+18,y+29); ctx.restore();
  }
  function neatAnswer(a,keys){ return smartAnswer(a,keys); }
  function makeNeatVisaCanvas(a,p,photo){
    var cv=document.createElement('canvas'); cv.width=1240; cv.height=1754; var ctx=cv.getContext('2d');
    ctx.fillStyle='#fff'; ctx.fillRect(0,0,cv.width,cv.height);
    ctx.fillStyle='#111827'; ctx.font='700 36px Arial, Helvetica, sans-serif'; ctx.fillText('VISA APPLICATION FORM',70,82);
    ctx.fillStyle='#64748b'; ctx.font='500 17px Arial, Helvetica, sans-serif';
    ctx.fillText('VisaDoo'+(a.reference_code?'  ·  '+a.reference_code:''),72,116);
    var visa=visaById(a.visa_type); var cn=visaCountryName(a); ctx.fillText(((visa&&visa.name)||a.visa_type||'') + (cn ? ' ('+cn+')' : ''),72,145);

    // Passport/application photo: fixed, uncluttered box with cover-crop and no distortion.
    var px=900,py=56,pw=250,ph=315;
    ctx.strokeStyle='#aeb7c4'; ctx.lineWidth=2; ctx.strokeRect(px,py,pw,ph);
    if(photo){
      var iw=photo.naturalWidth||photo.width, ih=photo.naturalHeight||photo.height;
      if(iw&&ih){ var r=Math.max(pw/iw,ph/ih), sw=pw/r, sh=ph/r, sx=(iw-sw)/2, sy=(ih-sh)/2;
        ctx.save(); ctx.beginPath(); ctx.rect(px,py,pw,ph); ctx.clip(); ctx.drawImage(photo,sx,sy,sw,sh,px,py,pw,ph); ctx.restore(); }
    } else { ctx.fillStyle='#94a3b8'; ctx.font='600 18px Arial'; ctx.textAlign='center'; ctx.fillText('APPLICANT PHOTO',px+pw/2,py+ph/2); ctx.textAlign='left'; }

    var x=70,w=800,y=200;
    neatSection(ctx,'Personal Details',x,y,w); y+=78;
    neatRow(ctx,'First name',p.first_name||p.full_name,x,y,w); y+=58;
    neatRow(ctx,'Last name',p.last_name,x,y,w); y+=58;
    neatRow(ctx,'Date of birth',pdfDate(p.date_of_birth),x,y,w); y+=58;
    neatRow(ctx,'Gender',p.gender,x,y,w); y+=58;
    neatRow(ctx,'Nationality',p.nationality,x,y,w); y+=58;
    neatRow(ctx,"Father's name",p.father_name,x,y,w); y+=58;
    neatRow(ctx,"Mother's name",p.mother_name,x,y,w); y+=78;

    neatSection(ctx,'Passport Details',x,y,w); y+=78;
    neatRow(ctx,'Passport number',p.passport_number,x,y,w); y+=58;
    neatRow(ctx,'Issue date',pdfDate(p.passport_issue_date),x,y,w); y+=58;
    neatRow(ctx,'Expiry date',pdfDate(p.passport_expiry),x,y,w); y+=58;
    neatRow(ctx,'Issuing country',p.passport_issuing_country,x,y,w); y+=78;

    neatSection(ctx,'Contact & Visa Details',x,y,w); y+=78;
    neatRow(ctx,'Mobile',p.phone,x,y,w); y+=58;
    neatRow(ctx,'Email',p.email,x,y,w); y+=58;
    neatRow(ctx,'Visa',((visa&&visa.name)||a.visa_type||'') + (cn ? ' ('+cn+')' : ''),x,y,w); y+=58;
    neatRow(ctx,'Status',a.status||'',x,y,w); y+=58;

    // Use saved answers only; never guess. Print important extra travel details if present.
    var extras=[
      ['Place of birth',neatAnswer(a,['place_of_birth','birth_place'])],
      ['Marital status',neatAnswer(a,['marital_status'])],
      ['Residential address',neatAnswer(a,['address','residential_address','current_address'])],
      ['Occupation',neatAnswer(a,['occupation','profession'])],
      ['Purpose of visit',neatAnswer(a,['purpose_of_visit','travel_purpose','purpose'])],
      ['Arrival date',pdfDate(neatAnswer(a,['arrival_date','date_of_arrival','travel_date']))],
      ['Intended stay',neatAnswer(a,['intended_length_of_stay','length_of_stay','stay_duration'])],
      ['Hotel / host',neatAnswer(a,['hotel_name','accommodation_name'])],
      ['Flight / airline',neatAnswer(a,['airline','flight_number','ship_or_airline'])]
    ].filter(function(it){return String(it[1]||'').trim();});
    if(extras.length && y<1550){
      y+=20; neatSection(ctx,'Travel / Additional Details',x,y,w); y+=72;
      extras.slice(0,4).forEach(function(it){ neatRow(ctx,it[0],it[1],x,y,w); y+=54; });
    }
    ctx.fillStyle='#94a3b8'; ctx.font='500 14px Arial'; ctx.fillText('Generated from saved application data. Please verify before submission.',70,1700);
    return cv;
  }
  function exactJapanText(ctx, value, xPdf, yTopPdf, bold, size){
    if(value==null || String(value).trim()==='') return;
    var sx=1241/595.32, sy=1754/841.92;
    canvasText(ctx,String(value).toUpperCase(),xPdf*sx,yTopPdf*sy,bold!==false,(size||8)*sy);
  }
  function japanPdfCheck(ctx,cxPdf,cyTopPdf,on){
    if(!on) return;
    var sx=1241/595.32, sy=1754/841.92;
    japanPixelCheck(ctx,cxPdf*sx,cyTopPdf*sy,true);
  }
  function japanPixelCheck(ctx,cx,cy,on){
    if(!on) return;
    ctx.save(); ctx.strokeStyle='#111'; ctx.lineWidth=1.7; ctx.lineCap='round'; ctx.lineJoin='round';
    ctx.beginPath(); ctx.moveTo(cx-4,cy); ctx.lineTo(cx-1,cy+4); ctx.lineTo(cx+5,cy-5); ctx.stroke(); ctx.restore();
  }
  function stampExactJapanPage1(ctx,p,a,photo){
    // Japan official template: values are positioned inside the underline area AFTER each printed label.
    // Coordinates are PDF points (top-origin), converted by exactJapanText().
    exactJapanText(ctx,p.last_name,216,218,true,10);
    exactJapanText(ctx,p.first_name||p.full_name,289,239,true,10);
    exactJapanText(ctx,smartAnswer(a,['other_names']),95,270,true,10);
    exactJapanText(ctx,pdfDate(p.date_of_birth),128,289,true,10);
    var bCity=smartAnswer(a,['birthplace_city']), bState=smartAnswer(a,['birthplace_state']), bCountry=smartAnswer(a,['birthplace_country']);
    var bComb=[bCity,bState,bCountry].filter(Boolean).join(', ')||smartAnswer(a,['place_of_birth','birth_place']);
    exactJapanText(ctx,bComb,270,289,true,10);

    var gender=String(p.gender||'').toLowerCase();
    japanPdfCheck(ctx,118.8,306.0,gender==='male'); japanPdfCheck(ctx,176.5,306.0,gender==='female');
    var ms=String(smartAnswer(a,['marital_status'])||'').toLowerCase();
    japanPdfCheck(ctx,335.9,306.0,ms==='single'); japanPdfCheck(ctx,394.8,306.0,ms==='married');
    japanPdfCheck(ctx,462.9,306.0,ms==='widowed'); japanPdfCheck(ctx,528.6,306.0,ms==='divorced');
    exactJapanText(ctx,p.nationality,184,329,true,10);
    exactJapanText(ctx,smartAnswer(a,['former_nationality','former_nationalities']),320,349,true,10);
    exactJapanText(ctx,smartAnswer(a,['government_id','id_number']),258,369,true,10);

    var pt=String(smartAnswer(a,['passport_type'])||'ordinary').toLowerCase();
    japanPdfCheck(ctx,191.4,386.0,pt==='diplomatic'); japanPdfCheck(ctx,249.1,386.0,pt==='official');
    japanPdfCheck(ctx,315.4,386.0,pt==='ordinary'); japanPdfCheck(ctx,370.2,386.0,pt==='other');
    exactJapanText(ctx,p.passport_number,451,390,true,10);
    exactJapanText(ctx,smartAnswer(a,['passport_place_of_issue','place_of_issue'])||p.passport_issuing_country,132,408,true,10);
    exactJapanText(ctx,pdfDate(p.passport_issue_date),475,408,true,10);
    exactJapanText(ctx,smartAnswer(a,['issuing_authority']),147,428,true,10);
    exactJapanText(ctx,pdfDate(p.passport_expiry),481,428,true,10);
    exactJapanText(ctx,smartAnswer(a,['certificate_of_eligibility']),191,448,true,10);
    exactJapanText(ctx,smartAnswer(a,['purpose_of_visit','travel_purpose','purpose']),272,467,true,10);
    exactJapanText(ctx,smartAnswer(a,['intended_length_of_stay','length_of_stay','stay_duration']),218,487,true,10);
    exactJapanText(ctx,pdfDate(smartAnswer(a,['arrival_date','date_of_arrival','travel_date'])),417,487,true,10);
    exactJapanText(ctx,smartAnswer(a,['port_of_entry']),177,507,true,10);
    exactJapanText(ctx,smartAnswer(a,['airline','flight_number','ship_or_airline']),416,507,true,10);
    exactJapanText(ctx,smartAnswer(a,['hotel_name','accommodation_name']),125,548,true,10);
    exactJapanText(ctx,smartAnswer(a,['hotel_phone','accommodation_phone']),441,548,true,10);
    exactJapanText(ctx,smartAnswer(a,['hotel_address','accommodation_address']),136,580,true,10);
    exactJapanText(ctx,smartAnswer(a,['previous_japan_stays','previous_stays']),278,600,true,10);
    exactJapanText(ctx,smartAnswer(a,['address','residential_address','current_address']),136,651,true,10);
    exactJapanText(ctx,smartAnswer(a,['telephone','tel']),116,670,true,10);
    exactJapanText(ctx,p.phone,270,670,true,10);
    exactJapanText(ctx,p.email,130,689,true,10);
    var occ=[smartAnswer(a,['occupation','profession']),smartAnswer(a,['position'])].filter(Boolean).join(' - ');
    exactJapanText(ctx,occ,282,709,true,10);
    exactJapanText(ctx,smartAnswer(a,['employer_name','company_name']),125,751,true,10);
    exactJapanText(ctx,smartAnswer(a,['employer_phone','company_phone']),441,751,true,10);
    exactJapanText(ctx,smartAnswer(a,['employer_address','company_address']),136,783,true,10);
    // Personal photo intentionally remains blank on the Japan form.
  }

  async function getJapanPdfSignature(a){
    var docs=(a&&a.documents)||[], d=null;
  ...
  function drawExactJapanSignature(ctx,signature){
    if(!signature) return;
    var sx=1241/595.32, sy=1754/841.92;
    // Signature sits directly above the official signature line (y=704), never in the privacy paragraph.
    var x=390*sx, y=674*sy, w=145*sx, h=27*sy;
    var iw=signature.naturalWidth||signature.width, ih=signature.naturalHeight||signature.height; if(!iw||!ih)return;
    var r=Math.min(w/iw,h/ih),dw=iw*r,dh=ih*r,dx=x+(w-dw)/2,dy=y+(h-dh)/2;
    ctx.drawImage(signature,dx,dy,dw,dh);
  }
  function stampExactJapanPage2(ctx,p,a,signature){
    exactJapanText(ctx,smartAnswer(a,['partner_occupation']),260,60,true,10);
    var gNa=String(smartAnswer(a,['guarantor_na'])).toLowerCase()==='yes';
    if(!gNa){
      // Page 2: keep each value immediately after the printed label with only a small readable gap.
      exactJapanText(ctx,smartAnswer(a,['guarantor_name']),124,98,true,10); exactJapanText(ctx,smartAnswer(a,['guarantor_phone']),439,98,true,10);
      exactJapanText(ctx,smartAnswer(a,['guarantor_address']),134,129,true,10); exactJapanText(ctx,pdfDate(smartAnswer(a,['guarantor_dob'])),155,148,true,10);
      var gg=String(smartAnswer(a,['guarantor_gender'])||'').toLowerCase(); japanPdfCheck(ctx,331.7,145.2,gg==='male'); japanPdfCheck(ctx,389.4,145.2,gg==='female');
      exactJapanText(ctx,smartAnswer(a,['guarantor_relationship']),212,167,true,10); exactJapanText(ctx,smartAnswer(a,['guarantor_occupation']),271,186,true,10);
      exactJapanText(ctx,[smartAnswer(a,['guarantor_nationality']),smartAnswer(a,['guarantor_immigration_status'])].filter(Boolean).join(', '),257,205,true,10);
    }
    var iNa=String(smartAnswer(a,['inviter_na'])).toLowerCase()==='yes';
    if(!iNa){
      exactJapanText(ctx,smartAnswer(a,['inviter_name']),124,242,true,10); exactJapanText(ctx,smartAnswer(a,['inviter_phone']),439,242,true,10);
      exactJapanText(ctx,smartAnswer(a,['inviter_address']),134,273,true,10); exactJapanText(ctx,pdfDate(smartAnswer(a,['inviter_dob'])),155,292,true,10);
      var ig=String(smartAnswer(a,['inviter_gender'])||'').toLowerCase(); japanPdfCheck(ctx,331.7,289.6,ig==='male'); japanPdfCheck(ctx,389.4,289.6,ig==='female');
      exactJapanText(ctx,smartAnswer(a,['inviter_relationship']),212,312,true,10); exactJapanText(ctx,smartAnswer(a,['inviter_occupation']),271,331,true,10);
      exactJapanText(ctx,[smartAnswer(a,['inviter_nationality']),smartAnswer(a,['inviter_immigration_status'])].filter(Boolean).join(', '),257,350,true,10);
    }
    exactJapanText(ctx,smartAnswer(a,['remarks','special_circumstances']),285,369,true,10);
  ...
  async function generateSmartVisaPdf(a){
    ...
    toast('Exact Japan visa form generated. Photo box left blank.');
  }

  // ============================================================
  //  SMART CUSTOMER REUSE + AI VISA ASSISTANT
  // ============================================================
  var mySavedProfiles=[];
  function appAnswerValue(app,key){
    var arr=(app&&Array.isArray(app.answers))?app.answers:[];
    for(var i=0;i<arr.length;i++){ if(arr[i]&&arr[i].q===key) return arr[i].value||''; }
    return '';
  }
  function profileFromApplication(a){
    if(!a) return null;
    var full=(a.full_name||'').trim();
    var first=appAnswerValue(a,'passport_first_name')||full.split(/\s+/)[0]||'';
    var last=appAnswerValue(a,'passport_last_name')||full.split(/\s+/).slice(1).join(' ');
    return { id:a.id, label:(full||a.email||'Saved traveller')+' · '+new Date(a.created_at||Date.now()).toLocaleDateString(),
      full_name:full, first_name:first, last_name:last, father_name:appAnswerValue(a,'father_name'), mother_name:appAnswerValue(a,'mother_name'),
      gender:appAnswerValue(a,'gender'), passport_number:a.passport_number||'', passport_issuing_country:a.passport_issuing_country||'',
      nationality:a.nationality||'', date_of_birth:a.date_of_birth||'', passport_issue_date:appAnswerValue(a,'passport_issue_date')||'',
      passport_expiry:a.passport_expiry||'', phone:a.phone||'', email:a.email||'',
      birthplace_city:appAnswerValue(a,'birthplace_city'), birthplace_state:appAnswerValue(a,'birthplace_state'), birthplace_country:appAnswerValue(a,'birthplace_country'),
      marital_status:appAnswerValue(a,'marital_status'), former_nationality:appAnswerValue(a,'former_nationality'), government_id:appAnswerValue(a,'government_id'),
      passport_type:appAnswerValue(a,'passport_type'), issuing_authority:appAnswerValue(a,'issuing_authority'),
      residential_address:appAnswerValue(a,'residential_address')||appAnswerValue(a,'address')||appAnswerValue(a,'current_address'),
  ...
  function applySavedProfile(p){
    if(!p) return;
    setFieldIfPresent('full_name',p.full_name); setFieldIfPresent('first_name',p.first_name); setFieldIfPresent('last_name',p.last_name);
    setFieldIfPresent('father_name',p.father_name); setFieldIfPresent('mother_name',p.mother_name); setFieldIfPresent('gender',p.gender);
    setFieldIfPresent('passport_number',p.passport_number); setFieldIfPresent('passport_issuing_country',p.passport_issuing_country);
    setFieldIfPresent('nationality',p.nationality); setFieldIfPresent('date_of_birth',p.date_of_birth); setFieldIfPresent('passport_issue_date',p.passport_issue_date);
    setFieldIfPresent('passport_expiry',p.passport_expiry);
    setFieldIfPresent('birthplace_city',p.birthplace_city); setFieldIfPresent('birthplace_state',p.birthplace_state); setFieldIfPresent('birthplace_country',p.birthplace_country);
    setFieldIfPresent('marital_status',p.marital_status); setFieldIfPresent('former_nationality',p.former_nationality); setFieldIfPresent('government_id',p.government_id);
    setFieldIfPresent('passport_type',p.passport_type); setFieldIfPresent('issuing_authority',p.issuing_authority);
    ...
    toast('Saved traveller details filled. Please review before submitting.');
  }
  function loadMySavedTravellerProfiles(){
    var card=document.getElementById('smartReuseCard'); if(!card||!state.user) return;
    sb.from('applications').select('id,full_name,email,phone,passport_number,passport_issuing_country,nationality,date_of_birth,passport_expiry,answers,created_at').eq('user_id',state.user.id).order('created_at',{ascending:false}).limit(25).then(function(r){
      if(r.error||!r.data||!r.data.length) return;
      var seen={}, out=[];
      r.data.forEach(function(a){ var p=profileFromApplication(a); var k=(p.passport_number||p.full_name||p.id).toLowerCase(); if(!seen[k]){seen[k]=1; out.push(p);} });
      mySavedProfiles=out; if(!out.length) return;
      var sel=document.getElementById('smartReuseSelect'); card.hidden=false;
      sel.innerHTML=out.map(function(p,i){return '<option value="'+i+'">'+esc(p.label)+'</option>';}).join('');
      document.getElementById('smartReuseText').textContent=out.length===1?'We found your previous visa details.':'Choose a traveller saved from your previous applications.';
      document.getElementById('smartReuseBtn').onclick=function(){ applySavedProfile(out[Number(sel.value)||0]); };
    }).catch(function(){ /* history reuse is optional */ });
  }

  var aiCustResults=[], aiSelected=[];
  function aiCustomerDisplay(c){ return (c.full_name||c.email||'Customer')+(c.phone?' · '+c.phone:''); }
  function aiFindCustomers(q){
    q=(q||'').trim(); if(q.length<2) return Promise.resolve([]);
    var safe=q.replace(/[%_,()]/g,' ');
    return sb.from('customers').select('id,full_name,email,phone,date_of_birth,passport_issuing_country,state,notes,created_at').or('full_name.ilike.%'+safe+'%,email.ilike.%'+safe+'%,phone.ilike.%'+safe+'%').limit(20).then(function(r){ return r.error?[]:(r.data||[]); });
  }
  function aiLatestApplication(customerId){
    return sb.from('applications').select('id,reference_code,visa_type,status,full_name,email,phone,passport_number,passport_issuing_country,nationality,date_of_birth,passport_expiry,answers,created_at').eq('customer_id',customerId).order('created_at',{ascending:false}).limit(1).then(function(r){return (!r.error&&r.data&&r.data[0])?r.data[0]:null;});
  }
  function paintAiSelected(){
    var box=document.getElementById('aiSelectedPeople'); if(!box) return;
    if(!aiSelected.length){ box.innerHTML='<p class="phint">No traveller selected yet. Search and add a customer or family member.</p>'; return; }
    box.innerHTML=aiSelected.map(function(x,i){ return '<div class="ai-person-chip"><div><b>'+esc(x.profile.full_name||x.customer.full_name||'Traveller')+'</b><small>'+esc(x.profile.passport_number?('Passport '+x.profile.passport_number):(x.customer.email||x.customer.phone||''))+'</small></div><button type="button" data-airm="'+i+'" aria-label="Remove">×</button></div>'; }).join('');
    box.querySelectorAll('[data-airm]').forEach(function(b){ b.onclick=function(){ aiSelected.splice(Number(b.getAttribute('data-airm')),1); paintAiSelected(); }; });
  }
  function japanFormHtml(p){
    p=p||{}; var today=new Date().toISOString().slice(0,10);
    function row(label,val){ return '<div class="jvf-row"><span>'+esc(label)+'</span><b>'+esc(val||'________________________')+'</b></div>'; }
    return '<div class="japan-form-preview"><div class="jvf-head"><h2>VISA APPLICATION FORM TO ENTER JAPAN</h2><span>AI pre-filled working copy</span></div>'+row('Surname (as shown in passport)',p.last_name)+row('Given and middle names',p.first_name||p.full_name)+row('Date of birth',p.date_of_birth)+row('Place of birth','')+row('Sex',p.gender)+row('Nationality or citizenship',p.nationality)+row('Passport No.',p.passport_number)+row('Place of issue',p.passport_issuing_country)+row('Date of issue',p.passport_issue_date)+row('Date of expiry',p.passport_expiry)+row('Purpose of visit','TOURISM')+row('Current residential address','')+row('Mobile No.',p.phone)+row('E-Mail',p.email)+row('Current profession / occupation','')+row('Date of application',today)+'<div class="jvf-note">Fields not stored in the customer profile remain blank so staff can complete them safely.</div></div>';
  }
  function renderAiFormPreview(){
    var area=document.getElementById('aiFormPreview'); if(!area) return;
    if(!aiSelected.length){ area.innerHTML='<div class="empty-state"><p>Select at least one traveller first.</p></div>'; return; }
    area.innerHTML=aiSelected.map(function(x){return japanFormHtml(x.profile);}).join('<div class="ai-page-break"></div>');
    var print=document.getElementById('aiPrintForms'); if(print) print.hidden=false;
  }
  function aiBuildContext(){
    return aiSelected.map(function(x){ var p=x.profile; return {name:p.full_name,email:p.email,phone:p.phone,nationality:p.nationality,passport_number:p.passport_number,passport_expiry:p.passport_expiry,date_of_birth:p.date_of_birth,last_visa:x.application&&x.application.visa_type,last_status:x.application&&x.application.status}; });
  }
  function aiFallbackAnswer(q){
    var low=q.toLowerCase(), ctx=aiBuildContext();
    if(!ctx.length) return 'Search and add a customer first. Then I can summarise saved details, identify missing fields, or prepare a visa form.';
    if(low.indexOf('missing')>-1 || low.indexOf('complete')>-1){
      return ctx.map(function(p){var miss=[]; ['name','email','phone','nationality','passport_number','passport_expiry','date_of_birth'].forEach(function(k){if(!p[k]) miss.push(k.replace(/_/g,' '));}); return p.name+': '+(miss.length?('missing '+miss.join(', ')):'core saved details are complete.');}).join('\n');
    }
    if(low.indexOf('passport')>-1 || low.indexOf('detail')>-1 || low.indexOf('show')>-1){ return ctx.map(function(p){return p.name+' — passport '+(p.passport_number||'not saved')+', nationality '+(p.nationality||'not saved')+', expiry '+(p.passport_expiry||'not saved')+'.';}).join('\n'); }
    if(low.indexOf('form')>-1 || low.indexOf('japan')>-1){ renderAiFormPreview(); return 'I prepared the Japan working-copy form below using the saved traveller details. Blank fields still need staff review.'; }
    return 'I can use the selected customer records to summarise details, flag missing information, and prepare the Japan visa working-copy form. Ask “show details”, “what is missing?”, or “create Japan form”.';
  }
  function askBackendAi(q){
    var ctx=aiBuildContext(); var msg=q+'\n\nSECURE STAFF CONTEXT (only use this to answer this staff request; do not invent missing values):\n'+JSON.stringify(ctx);
    return fetch('/.netlify/functions/ai-chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({messages:[{role:'user',content:msg}],country:'Back-office visa operations'})}).then(function(r){if(!r.ok) throw new Error('AI unavailable'); return r.json();}).then(function(d){return d.reply||aiFallbackAnswer(q);}).catch(function(){return aiFallbackAnswer(q);});
  }
  function renderAiVisaAssistant(){
    if(state.role!=='admin'){ go(defaultStaffView()); return; }
    aiCustResults=[]; aiSelected=[];
    root.innerHTML='<div class="app-main">'+adminSections('aiassistant')+
      '<div class="app-head"><div><h1>AI Visa Assistant</h1><p>Reuse saved customer details, build a family/traveller set, check missing data and create a pre-filled visa working copy.</p></div></div>'+ 
      '<div class="ai-workspace">'+
        '<section class="panel ai-customer-panel"><h3>1. Find customer / family member</h3><div class="ai-search-row"><input id="aiCustSearch" placeholder="Search name, email or phone"><button class="btn btn-primary" id="aiCustSearchBtn" type="button">Search</button></div><div id="aiCustResults" class="ai-cust-results"><p class="phint">Search an existing customer. Add each family member who is travelling.</p></div><h3 style="margin-top:18px">Selected travellers</h3><div id="aiSelectedPeople"></div></section>'+ 
        '<section class="panel ai-chat-panel"><div class="ai-chat-head"><div class="ai-avatar">AI</div><div><b>VisaDoo Back-office Assistant</b><small>Uses selected customer records only</small></div></div><div id="aiChatLog" class="ai-chat-log"><div class="ai-msg assistant">Select a customer, then ask me to show saved details, find missing information, or create a Japan visa form.</div></div><div class="ai-chat-compose"><textarea id="aiChatInput" rows="2" placeholder="Ask about the selected customer(s)…"></textarea><button class="btn btn-primary" id="aiChatSend" type="button">Send</button></div><div class="ai-quick"><button type="button" data-aiq="Show saved customer details">Show details</button><button type="button" data-aiq="What important visa fields are missing?">Missing fields</button><button type="button" data-aiq="Create Japan visa form">Create Japan form</button></div></section>'+ 
      '</div>'+ 
      '<section class="panel ai-form-panel"><div class="ai-form-title"><div><h3>2. AI Form Generator</h3><p class="phint">The sample Japan form is included as the reference. Generated output is a reviewable working copy; staff must verify it before official submission.</p></div><div><a class="btn btn-ghost" href="assets/form-templates/japan-visa-reference.pdf" target="_blank" rel="noopener">Open reference PDF</a> <button class="btn btn-primary" id="aiGenerateForms" type="button">Generate from saved details</button> <button class="btn btn-ghost" id="aiPrintForms" type="button" hidden>Print / Save PDF</button></div></div><div id="aiFormPreview" class="ai-form-preview"><div class="empty-state"><p>Select traveller(s), then generate the form.</p></div></div></section>'+ 
    '</div>';
    wireAdminSections(); paintAiSelected();
    function doSearch(){ var q=document.getElementById('aiCustSearch').value; var area=document.getElementById('aiCustResults'); area.innerHTML='<p class="phint">Searching…</p>'; aiFindCustomers(q).then(function(rows){ aiCustResults=rows; if(!rows.length){area.innerHTML='<p class="phint">No matching customers.</p>'; return;} area.innerHTML=rows.map(function(c,i){return '<button class="ai-cust-row" type="button" data-aic="'+i+'"><span><b>'+esc(c.full_name||'(no name)')+'</b><small>'+esc([c.email,c.phone].filter(Boolean).join(' · '))+'</small></span><em>+ Add</em></button>';}).join(''); area.querySelectorAll('[data-aic]').forEach(function(b){ b.onclick=function(){ var c=aiCustResults[Number(b.getAttribute('data-aic'))]; if(aiSelected.some(function(x){return x.customer.id===c.id;})){toast('That traveller is already selected.');return;} b.disabled=true; b.querySelector('em').textContent='Loading…'; aiLatestApplication(c.id).then(function(a){ var p=a?profileFromApplication(a):{full_name:c.full_name||'',email:c.email||'',phone:c.phone||'',date_of_birth:c.date_of_birth||'',passport_issuing_country:c.passport_issuing_country||''}; aiSelected.push({customer:c,application:a,profile:p}); paintAiSelected(); b.querySelector('em').textContent='Added'; }); }; }); }); }
    document.getElementById('aiCustSearchBtn').onclick=doSearch; document.getElementById('aiCustSearch').onkeydown=function(e){if(e.key==='Enter'){e.preventDefault();doSearch();}};
    function sendQ(q){ q=(q||'').trim(); if(!q)return; var log=document.getElementById('aiChatLog'); log.innerHTML+='<div class="ai-msg user">'+esc(q)+'</div><div class="ai-msg assistant thinking">Checking saved records…</div>'; log.scrollTop=log.scrollHeight; var thinking=log.querySelector('.thinking:last-child'); askBackendAi(q).then(function(ans){if(thinking){thinking.classList.remove('thinking');thinking.textContent=ans;} log.scrollTop=log.scrollHeight;}); }
    document.getElementById('aiChatSend').onclick=function(){var i=document.getElementById('aiChatInput');var q=i.value;i.value='';sendQ(q);};
    document.getElementById('aiChatInput').onkeydown=function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();var q=this.value;this.value='';sendQ(q);}};
    document.querySelectorAll('[data-aiq]').forEach(function(b){b.onclick=function(){sendQ(b.getAttribute('data-aiq'));};});
    document.getElementById('aiGenerateForms').onclick=renderAiFormPreview;
    document.getElementById('aiPrintForms').onclick=function(){ document.body.classList.add('print-ai-form'); window.print(); setTimeout(function(){document.body.classList.remove('print-ai-form');},400); };
  }
  ...
  function render(){
    ...
    if((v==='team'||v==='brand'||v==='emailcfg'||v==='customers'||v==='aiassistant'||v==='automations'||v==='templates'||v==='msghistory'||v==='audit') && state.role!=='admin') v=defaultStaffView();
    ...
    else if(v==='aiassistant') renderAiVisaAssistant();
