(function () {
  ...
  function renderError(title,message){
    root.innerHTML='<section class="country-page-error"><div><div class="country-error-icon">!</div><h1>'+esc(title)+'</h1><p>'+esc(message)+'</p><a href="/articles/articles.html" class="btn btn-primary btn-lg">See all articles</a></div></section>';
  }
      ...
      '<article class="article-detail">'+
        '<header class="article-hero'+(cover?'':' article-hero--plain')+'">'+
          ...
          '<div class="container article-hero-copy">'+
            '<a href="/articles/articles.html" class="article-back"><span>←</span> Back to the journal</a>'+
  ...
  fetchJson('/rest/v1/articles?status=eq.published&slug=eq.'+encodeURIComponent(articleSlug)+'&select=*').then(function(rows){
    ...
    return fetchJson('/rest/v1/articles?status=eq.published&past_slugs=cs.%7B'+encodeURIComponent(articleSlug)+'%7D&select=slug&limit=1').then(function(moved){
      if(moved.length){
        ...
        window.location.replace(local?('/articles/article.html?slug='+encodeURIComponent(moved[0].slug)):('/article/'+encodeURIComponent(moved[0].slug)));
        return;
      }
