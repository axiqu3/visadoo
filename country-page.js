...
(function () {
    ...
    var choices=sortedVisas.map(function(visa,index){
      ...
      return '<label class="uae-travel-option-card'+(index===0?' selected':'')+'">'+
        ...
        '<div class="uae-travel-card-specs-panel">'+
          ...
          '<div class="uae-travel-spec-item">'+
            '<small>ENTRY</small>'+
            '<b>'+esc(visa.sub||visa.category||'See details')+'</b>'+
          '</div>'+
          '<div class="uae-travel-spec-item">'+
  ...
  function uaeHeroBanner(country, image, visas) {
    ...
    var fastest = visasArray.reduce(function(acc, v){
      var days = parseFloat(v.processing_time);
    ...
    var priceTextStr = fromPrice ? (currency + ' ' + fromPrice) : 'AED 350';
    var daysTextStr = fastest ? (fastest + ' business days') : '2 business days';
  ...
  function exploreOtherVisasSection(otherCountries) {
    ...
    list = list.filter(function(c){ return c.slug !== currentSlug; }).slice(0, 4);
  ...
  Promise.all([
    ...
    fetchJson('/rest/v1/countries?active=eq.true&slug=neq.'+encodeURIComponent(slug)+'&limit=5').catch(function(){ return []; })
  ]).then(function(data){
